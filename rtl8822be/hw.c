...
void rtl8822be_set_hw_reg(struct ieee80211_hw *hw, u8 variable, u8 *val)
{
	...
	case HW_VAR_ACK_PREAMBLE: {
		...
		val8 = rtl_read_byte(rtlpriv, REG_WMAC_TRXPTCL_CTL_8822B + 2);
		val8 |= BIT(4) | BIT(5);
		...
		rtl_write_byte(rtlpriv, REG_WMAC_TRXPTCL_CTL_8822B + 2, val8);
...
static void _rtl8822be_enable_aspm_back_door(struct ieee80211_hw *hw)
{
	...
	pci_write_config_byte(rtlpci->pdev, 0x70f, (tmp & (~0x38)) | 0x28);
				    ...
				    struct rtl_int *intvec)
{
	...
	rtl_write_dword(rtlpriv, REG_HISR0_8822B, intvec->inta);

	intvec->intb =
		rtl_read_dword(rtlpriv, REG_HISR1_8822B) & rtlpci->irq_mask[1];
	rtl_write_dword(rtlpriv, REG_HISR1_8822B, intvec->intb);

	intvec->intd =
		rtl_read_dword(rtlpriv, REG_HISR3_8822B) & rtlpci->irq_mask[3];
	rtl_write_dword(rtlpriv, REG_HISR3_8822B, intvec->intd);
}
					   ...
					   u8 rssi_level, u8 is_update_bw)
{
	...
	sta_entry->cmn_info.ra_info.ramask = (((u64)ratr_bitmap_msb<<32) | ((u64)ratr_bitmap));
	...
	sta_entry->cmn_info.ra_info.is_support_sgi = b_shortgi;

			     ...
			     struct sk_buff *skb)
{
	...
	status->ppdu_cnt = GET_RX_DESC_PPDU_CNT(pdesc);
			    ...
			    u8 hw_queue, struct rtl_tcb_desc *ptcb_desc)
{
	...
	if (firstseg) {
		...
		if (sta_entry) {
			/* stbc/ldpc in cmn sta info */
			if (sta_entry->cmn_info.stbc_en)
				SET_TX_DESC_DATA_STBC(pdesc, 1);
			if (sta_entry->cmn_info.ldpc_en)
				SET_TX_DESC_DATA_LDPC(pdesc, 1);
		}
	}

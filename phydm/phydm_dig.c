...
boolean phydm_dig_go_up_check(void *p_dm_void)
{
	...
	max_cover_bond =
		DIG_MAX_BALANCE_MODE - p_dig_t->dig_upcheck_initial_value;
	...
	PHYDM_DBG(p_dm, DBG_DIG,
		  ("check_lv = %d, max_cover_bond = 0x%x\n",
		   p_dig_t->dig_go_up_check_level, max_cover_bond));
	...
	if (p_dig_t->dig_go_up_check_level == DIG_GOUPCHECK_LEVEL_0) {
		...
		ret = ((p_dig_t->dig_level0_ratio_reciprocal * dig_cover_cnt) >=
		       total_nhm_cnt) ?
			      true :
			      false;
		...
		for (i = 0; i <= 10; i++) {
			if ((max_cover_bond * 2) == ccx_info->nhm_th[i]) {
				for (j = (i + 1); j <= 11; j++)
					over_dig_cover_cnt +=
						ccx_info->nhm_result[j];
				break;
			}
		}
		ret = (p_dig_t->dig_level1_ratio_reciprocal *
			       over_dig_cover_cnt <
		       total_nhm_cnt) ?
			      true :
			      false;
		...
		if (!ret) {
			...
			p_dig_t->rx_gain_range_max =
				(rx_gain_range_max >= max_cover_bond - 6) ?
					(max_cover_bond - 6) :
					rx_gain_range_max;
...
void odm_fa_threshold_check(void *p_dm_void, boolean is_dfs_band,
			    boolean is_performance)
{
	...
	if (p_dm->is_linked && (is_performance || is_dfs_band)) {
		if ((p_dm->rx_tp >> 2) > p_dm->tx_tp && p_dm->rx_tp < 10 &&
		    p_dm->rx_tp > 1) { /*10Mbps & 1Mbps*/
			p_dig_t->fa_th[0] = 125;
			p_dig_t->fa_th[1] = 250;
			p_dig_t->fa_th[2] = 500;
		} else {
	...
	PHYDM_DBG(p_dm, DBG_DIG, ("FA_th={%d,%d,%d}\n", p_dig_t->fa_th[0],
				  p_dig_t->fa_th[1], p_dig_t->fa_th[2]));
}
...
void phydm_set_big_jump_step(void *p_dm_void, u8 current_igi)
{
	...
	for (i = 0; i <= p_dig_t->big_jump_step1; i++) {
		if ((current_igi + step1[i]) >
		    p_dig_t->big_jump_lmt[p_dig_t->agc_table_idx]) {
			if (i != 0)
				i = i - 1;
			break;
	...
	if (p_dm->support_ic_type & ODM_RTL8822B)
		odm_set_bb_reg(p_dm, BBREG_0x8c8, 0xe, i);
	...
	PHYDM_DBG(
		...
		("phydm_set_big_jump_step(): bigjump = %d (ori = 0x%x), LMT=0x%x\n",
		 i, p_dig_t->big_jump_step1,
		 p_dig_t->big_jump_lmt[p_dig_t->agc_table_idx]));
#endif
}

void odm_write_dig(void *p_dm_void, u8 current_igi)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_dig_struct *p_dig_t = &p_dm->dm_dig_table;
	struct phydm_adaptivity_struct *p_adaptivity =
	...
	if (p_dig_t->cur_ig_value != current_igi) {
		...
		if (p_dm->cck_new_agc &&
		    (p_dm->support_ic_type & ODM_IC_PHY_STATUE_NEW_TYPE))
			odm_set_bb_reg(p_dm, BBREG_0xa0c, 0x3f00,
				       (current_igi >> 1));
		#endif
		...
		/* Set IGI value */
		odm_set_bb_reg(p_dm, ODM_REG(IGI_A, p_dm), ODM_BIT(IGI, p_dm),
			       current_igi);

		#if (defined(PHYDM_COMPILE_ABOVE_2SS))
		if (p_dm->support_ic_type & PHYDM_IC_ABOVE_2SS)
			odm_set_bb_reg(p_dm, ODM_REG(IGI_B, p_dm),
				       ODM_BIT(IGI, p_dm), current_igi);
		#endif
...
void phydm_set_dig_val(void *p_dm_void, u32 *val_buf, u8 val_len)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void odm_pause_dig(void *p_dm_void, enum phydm_pause_type pause_type,
		   enum phydm_pause_level pause_level, u8 igi_value)
{
	...
	case PHYDM_PAUSE: {
		...
		if (p_dig_t->pause_lv_bitmap == 0) {
			p_dig_t->igi_backup =
				p_dig_t->cur_ig_value; /* Backup IGI value */
			PHYDM_DBG(p_dm, DBG_DIG,
				  ("Backup IGI=0x%x\n", p_dig_t->igi_backup));
		}

		p_dig_t->pause_dig_value[pause_level] =
			igi_value; /* Record IGI value */
		p_dig_t->pause_lv_bitmap |=
			BIT(pause_level); /* Update pause level bit-map*/
	...
	case PHYDM_RESUME: {
		...
		PHYDM_DBG(p_dm, DBG_DIG,
			  ("Pause @ IGI{Lv=0x%x}=%d)\n", max_level,
			   p_dig_t->pause_dig_value[max_level]));
...
void phydm_dig_init(void *p_dm_void)
{
	...
	p_dig_t->is_ignore_dig = false;
	p_dig_t->cur_ig_value = (u8)odm_get_bb_reg(p_dm, ODM_REG(IGI_A, p_dm),
						   ODM_BIT(IGI, p_dm));
	...
	if (p_dm->support_ic_type & ODM_RTL8822B)
		ret_value = odm_get_bb_reg(p_dm, BBREG_0x8c8, MASKLWORD);
	else if (p_dm->support_ic_type & ODM_RTL8197F)
		ret_value = odm_get_bb_reg(p_dm, BBREG_0xc74, MASKLWORD);
...
void phydm_dig_abs_boundary_decision(struct PHY_DM_STRUCT *p_dm,
				     boolean is_performance,
				     boolean is_dfs_band)
{
	struct phydm_dig_struct *p_dig_t = &p_dm->dm_dig_table;
	...
	if (!p_dm->is_linked) {
		...
		if (*p_dm->p_bb_op_mode ==
		    PHYDM_BALANCE_MODE) { /*service > 2 devices*/
			p_dig_t->dm_dig_max = DIG_MAX_BALANCE_MODE;
			#if (DIG_HW == 1)
			p_dig_t->dig_max_of_min = DIG_MIN_COVERAGE;
			#else
			p_dig_t->dig_max_of_min = DIG_MAX_OF_MIN_BALANCE_MODE;
			#endif
		} else if (*p_dm->p_bb_op_mode ==
			   PHYDM_PERFORMANCE_MODE) { /*service 1 devices*/
			p_dig_t->dm_dig_max = DIG_MAX_PERFORMANCE_MODE;
			p_dig_t->dig_max_of_min =
				DIG_MAX_OF_MIN_PERFORMANCE_MODE;
		}
	...
	PHYDM_DBG(p_dm, DBG_DIG,
		  ("Abs-bound{Max, Min}={0x%x, 0x%x}, Max_of_min =  0x%x\n",
		   p_dig_t->dm_dig_max, p_dig_t->dm_dig_min,
		   p_dig_t->dig_max_of_min));
}

void phydm_dig_dym_boundary_decision(struct PHY_DM_STRUCT *p_dm,
				     boolean is_performance)
{
	struct phydm_dig_struct *p_dig_t = &p_dm->dm_dig_table;
	u8 offset = 15, tmp_max = 0;

	PHYDM_DBG(p_dm, DBG_DIG, ("Offset=((%d))\n", offset));
	...
	if ((p_dm->support_ic_type & ODM_ANTDIV_SUPPORT) &&
		...
		if (p_dm->ant_div_type == CG_TRX_HW_ANTDIV ||
			...
			if (p_dig_t->ant_div_rssi_max > p_dig_t->dig_max_of_min)
				p_dig_t->rx_gain_range_min =
					p_dig_t->dig_max_of_min;
			else
				p_dig_t->rx_gain_range_min =
					(u8)p_dig_t->ant_div_rssi_max;

			PHYDM_DBG(
				p_dm, DBG_DIG,
				("AntDiv: Force Dyn-Min = 0x%x, rssi_max = 0x%x\n",
				 p_dig_t->rx_gain_range_min,
				 p_dig_t->ant_div_rssi_max));
		}
	}
...
void phydm_dig_abnormal_case(struct PHY_DM_STRUCT *p_dm, u8 current_igi,
			     boolean is_performance, boolean is_dfs_band)
{
	struct phydm_dig_struct *p_dig_t = &p_dm->dm_dig_table;
...
u8 phydm_dig_current_igi_by_fa_th(struct PHY_DM_STRUCT *p_dm, u8 current_igi,
				  u32 false_alm_cnt, u8 *step_size)
{
...
u8 phydm_dig_igi_start_value(struct PHY_DM_STRUCT *p_dm, boolean is_performance,
			     u8 current_igi, u32 false_alm_cnt,
			     boolean is_dfs_band)
{
	struct phydm_dig_struct *p_dig_t = &p_dm->dm_dig_table;
	u8 step_size[3] = {0};
	...
	PHYDM_DBG(p_dm, DBG_DIG, ("step_size = {-%d,  +%d, +%d}\n",
				  step_size[2], step_size[1], step_size[0]));

	PHYDM_DBG(p_dm, DBG_DIG, ("rssi_min = %d, pre_rssi_min = %d\n",
...
/* 4 Abnormal # beacon case */
...
#endif
				current_igi = phydm_dig_current_igi_by_fa_th(
					p_dm, current_igi, false_alm_cnt,
					step_size);
		}
	} else {
		/* 2 Before link */
		...
		if (first_dis_connect) {
			...
			PHYDM_DBG(
				p_dm, DBG_DIG,
				("First disconnect case: IGI does on-shot to lower bound\n"));
		} else {
			PHYDM_DBG(p_dm, DBG_DIG,
				  ("Pre_IGI=((0x%x)), FA=((%d))\n", current_igi,
				   false_alm_cnt));
			...
			current_igi = phydm_dig_current_igi_by_fa_th(
				p_dm, current_igi, false_alm_cnt, step_size);
		}
	}
...
void phydm_dig(void *p_dm_void)
{
	...
	if (odm_dig_abort(p_dm) == true) {
		p_dig_t->cur_ig_value =
			(u8)odm_get_bb_reg(p_dm, BBREG_0xc50, 0x7f);
		return;
	}
	...
	PHYDM_DBG(
		...
		("is_linked = %d, RSSI = %d, 1stConnect = %d, 1stDisconnect = %d\n",
		 p_dm->is_linked, p_dm->rssi_min, first_connect,
		 first_dis_connect));
	...
	/*init dym boundary*/
	p_dig_t->rx_gain_range_max =
		p_dig_t->dig_max_of_min; /*if no link, always stay at lower bound*/
	p_dig_t->rx_gain_range_min = p_dig_t->dm_dig_min;
	...
	current_igi = phydm_dig_igi_start_value(
		p_dm, is_performance, current_igi, false_alm_cnt, is_dfs_band);
...
void phydm_dig_lps_32k(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_dig_by_rssi_lps(void *p_dm_void)
{
#if (DM_ODM_SUPPORT_TYPE & (ODM_WIN | ODM_CE))
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	PHYDM_DBG(p_dm, DBG_DIG, ("%s p_falm_cnt->cnt_all = %d\n", __func__,
				  p_falm_cnt->cnt_all));
	PHYDM_DBG(p_dm, DBG_DIG,
		  ("%s p_dm->rssi_min = %d\n", __func__, p_dm->rssi_min));
	PHYDM_DBG(p_dm, DBG_DIG,
...
void phydm_false_alarm_counter_reg_reset(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		...
		odm_set_bb_reg(p_dm, BBREG_0xc0c, BIT(31), 1);
		odm_set_bb_reg(p_dm, BBREG_0xc0c, BIT(31), 0);
		odm_set_bb_reg(p_dm, BBREG_0xd00, BIT(27), 1);
		odm_set_bb_reg(p_dm, BBREG_0xd00, BIT(27), 0);

		/*update ofdm counter*/
		/*update page C counter*/
		odm_set_bb_reg(p_dm, BBREG_0xd00, BIT(31), 0);
		/*update page D counter*/
		odm_set_bb_reg(p_dm, BBREG_0xd00, BIT(31), 0);

		/*reset CCK CCA counter*/
		odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(13) | BIT(12), 0);
		odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(13) | BIT(12), 2);

		/*reset CCK FA counter*/
		odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(15) | BIT(14), 0);
		odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(15) | BIT(14), 2);

		/*reset CRC32 counter*/
		odm_set_bb_reg(p_dm, BBREG_0xf14, BIT(16), 1);
		odm_set_bb_reg(p_dm, BBREG_0xf14, BIT(16), 0);
	}
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
			...
			if (false_alm_cnt->cnt_ofdm_fail_pre >= 0x7fff) {
				/* reset OFDM */
				odm_set_bb_reg(p_dm, BBREG_0x808, BIT(29), 0);
				odm_set_bb_reg(p_dm, BBREG_0x808, BIT(29), 1);
				false_alm_cnt->cnt_ofdm_fail_pre = 0;
				PHYDM_DBG(p_dm, DBG_FA_CNT, ("Reset FA_cnt\n"));
			}
	#endif	/* #if (RTL8881A_SUPPORT == 1) */
		/* reset OFDM FA countner */
		odm_set_bb_reg(p_dm, BBREG_0x9a4, BIT(17), 1);
		odm_set_bb_reg(p_dm, BBREG_0x9a4, BIT(17), 0);

		/* reset CCK FA counter */
		odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(15), 0);
		odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(15), 1);
...
void phydm_false_alarm_counter_reg_hold(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		/*hold ofdm counter*/
		/*hold page C counter*/
		odm_set_bb_reg(p_dm, BBREG_0xc00, BIT(31), 1);
		/*hold page D counter*/
		odm_set_bb_reg(p_dm, BBREG_0xd00, BIT(31), 1);

		//hold cck counter
		odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(12), 1);
		odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(14), 1);
	}
}
...
void odm_false_alarm_counter_statistics(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		...
		false_alm_cnt->cnt_cck_crc32_error = odm_get_bb_reg(
			p_dm, ODM_REG_CCK_CRC32_ERROR_CNT_11N, MASKDWORD);
		false_alm_cnt->cnt_cck_crc32_ok = odm_get_bb_reg(
			p_dm, ODM_REG_CCK_CRC32_OK_CNT_11N, MASKDWORD);

		/* read OFDM CRC32 counter */
		ret_value = odm_get_bb_reg(p_dm, ODM_REG_OFDM_CRC32_CNT_11N,
					   MASKDWORD);
		...
		ret_value = odm_get_bb_reg(p_dm, ODM_REG_HT_CRC32_CNT_11N,
					   MASKDWORD);
		...
		{
			ret_value = odm_get_bb_reg(p_dm, ODM_REG_CCK_FA_LSB_11N,
						   MASKBYTE0);
			false_alm_cnt->cnt_cck_fail = ret_value;

			ret_value = odm_get_bb_reg(p_dm, ODM_REG_CCK_FA_MSB_11N,
						   MASKBYTE3);
			false_alm_cnt->cnt_cck_fail += (ret_value & 0xff) << 8;

			ret_value = odm_get_bb_reg(
				p_dm, ODM_REG_CCK_CCA_CNT_11N, MASKDWORD);
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		ret_value = odm_get_bb_reg(p_dm, ODM_REG_OFDM_FA_TYPE1_11AC,
					   MASKDWORD);
		false_alm_cnt->cnt_fast_fsync =
			((ret_value & 0xffff0000) >> 16);

		ret_value = odm_get_bb_reg(p_dm, ODM_REG_OFDM_FA_TYPE2_11AC,
					   MASKDWORD);
		false_alm_cnt->cnt_sb_search_fail = (ret_value & 0xffff);

		ret_value = odm_get_bb_reg(p_dm, ODM_REG_OFDM_FA_TYPE3_11AC,
					   MASKDWORD);
		false_alm_cnt->cnt_parity_fail = (ret_value & 0xffff);
		false_alm_cnt->cnt_rate_illegal =
			((ret_value & 0xffff0000) >> 16);

		ret_value = odm_get_bb_reg(p_dm, ODM_REG_OFDM_FA_TYPE4_11AC,
					   MASKDWORD);
		false_alm_cnt->cnt_crc8_fail = (ret_value & 0xffff);
		false_alm_cnt->cnt_mcs_fail = ((ret_value & 0xffff0000) >> 16);

		ret_value = odm_get_bb_reg(p_dm, ODM_REG_OFDM_FA_TYPE5_11AC,
					   MASKDWORD);
		false_alm_cnt->cnt_crc8_fail_vht = (ret_value & 0xffff);

		ret_value = odm_get_bb_reg(p_dm, ODM_REG_OFDM_FA_TYPE6_11AC,
					   MASKDWORD);
		false_alm_cnt->cnt_mcs_fail_vht = (ret_value & 0xffff);

		/* read OFDM FA counter */
		false_alm_cnt->cnt_ofdm_fail =
			odm_get_bb_reg(p_dm, ODM_REG_OFDM_FA_11AC, MASKLWORD);

		/* Read CCK FA counter */
		false_alm_cnt->cnt_cck_fail =
			odm_get_bb_reg(p_dm, ODM_REG_CCK_FA_11AC, MASKLWORD);

		/* read CCK/OFDM CCA counter */
		ret_value = odm_get_bb_reg(p_dm, ODM_REG_CCK_CCA_CNT_11AC,
					   MASKDWORD);
		false_alm_cnt->cnt_ofdm_cca = (ret_value & 0xffff0000) >> 16;
		false_alm_cnt->cnt_cck_cca = ret_value & 0xffff;

		/* read CCK CRC32 counter */
		ret_value = odm_get_bb_reg(p_dm, ODM_REG_CCK_CRC32_CNT_11AC,
					   MASKDWORD);
		...
		/* read OFDM CRC32 counter */
		ret_value = odm_get_bb_reg(p_dm, ODM_REG_OFDM_CRC32_CNT_11AC,
					   MASKDWORD);
		false_alm_cnt->cnt_ofdm_crc32_error =
			(ret_value & 0xffff0000) >> 16;
		false_alm_cnt->cnt_ofdm_crc32_ok = ret_value & 0xffff;
		...
		/* read HT CRC32 counter */
		ret_value = odm_get_bb_reg(p_dm, ODM_REG_HT_CRC32_CNT_11AC,
					   MASKDWORD);
		false_alm_cnt->cnt_ht_crc32_error =
			(ret_value & 0xffff0000) >> 16;
		false_alm_cnt->cnt_ht_crc32_ok = ret_value & 0xffff;
		...
		/* read VHT CRC32 counter */
		ret_value = odm_get_bb_reg(p_dm, ODM_REG_VHT_CRC32_CNT_11AC,
					   MASKDWORD);
...
#endif
		cck_enable =
			odm_get_bb_reg(p_dm, ODM_REG_BB_RX_PATH_11AC, BIT(28));
	...
	if (p_dm->support_ic_type != ODM_RTL8723D) {
		if (phydm_set_bb_dbg_port(p_dm, BB_DBGPORT_PRIORITY_1,
					  0x0)) { /*set debug port to 0x0*/
			false_alm_cnt->dbg_port0 =
				phydm_get_bb_dbg_port_value(p_dm);
			phydm_release_bb_dbg_port(p_dm);
		}

		if (phydm_set_bb_dbg_port(p_dm, BB_DBGPORT_PRIORITY_1,
					  adaptivity->adaptivity_dbg_port)) {
			if (p_dm->support_ic_type &
			    (ODM_RTL8723B | ODM_RTL8188E))
				false_alm_cnt->edcca_flag = (boolean)(
					(phydm_get_bb_dbg_port_value(p_dm) &
					 BIT(30)) >>
					30);
			else
		...
		false_alm_cnt->edcca_flag =
			(boolean)(odm_get_bb_reg(p_dm, BBREG_0x9a0, BIT(29)));
	}

	phydm_false_alarm_counter_reg_reset(p_dm_void);

	false_alm_cnt->time_fa_all = (false_alm_cnt->cnt_fast_fsync +
				      false_alm_cnt->cnt_sb_search_fail) *
					     12 +
				     (false_alm_cnt->cnt_parity_fail +
				      false_alm_cnt->cnt_rate_illegal) *
					     28 +
				     (false_alm_cnt->cnt_crc8_fail +
				      false_alm_cnt->cnt_crc8_fail_vht +
				      false_alm_cnt->cnt_mcs_fail_vht) *
					     36 +
				     false_alm_cnt->cnt_mcs_fail * 32 +
				     false_alm_cnt->cnt_cck_fail * 80;
	...
	PHYDM_DBG(
		...
		("[OFDM FA Detail] Parity_Fail = (( %d )), Rate_Illegal = (( %d )), HT_CRC8_fail = (( %d )), HT_Mcs_fail = (( %d )), Fast_Fsync = (( %d )), SB_Search_fail = (( %d )), VHT_CRC8_fail = (( %d )), VHT_Mcs_fail = (( %d ))\n",
		 false_alm_cnt->cnt_parity_fail,
		 false_alm_cnt->cnt_rate_illegal, false_alm_cnt->cnt_crc8_fail,
		 false_alm_cnt->cnt_mcs_fail, false_alm_cnt->cnt_fast_fsync,
		 false_alm_cnt->cnt_sb_search_fail,
		 false_alm_cnt->cnt_crc8_fail_vht,
		 false_alm_cnt->cnt_mcs_fail_vht));
	...
	PHYDM_DBG(p_dm, DBG_FA_CNT, ("[TOTAL]  CRC32 {error, ok}= {%d, %d}\n",
				     false_alm_cnt->cnt_crc32_error_all,
	...
	)
{
	...
	if (p_dm->support_ic_type & ODM_RTL8197F)
		current_time_stamp = odm_get_bb_reg(p_dm, BBREG_0x568, bMaskDWord);
	else
		return;
	...
	if (p_dm->support_ic_type & ODM_RTL8197F)		/*REG_PS_TIMER2*/
		odm_set_bb_reg(p_dm, BBREG_0x588, bMaskDWord, timeout);
	else {
		PHYDM_DBG(p_dm, DBG_DIG,
	...
	)
{
	...
	p_dig_t = &p_dm->dm_dig_table;
	p_falm_cnt = &p_dm->false_alm_cnt;
	reg_c50 = odm_get_bb_reg(p_dm, BBREG_0xc50, MASKBYTE0);
	...
	)
{
	...
	if (p_dig_t->tdma_dig_state == 1)
		...
		if (p_dm->support_ic_type & ODM_RTL8197F)		/*REG_FREERUN_CNT*/
			timestamp = odm_get_bb_reg(p_dm, BBREG_0x568, bMaskDWord);
		else {
			PHYDM_DBG(p_dm, DBG_DIG,
...
)
{
	...
	if (tab_sel == OFDM_AGC_TAB_0)
		odm_set_bb_reg(p_dm, BBREG_0xc70, 0x1e00, OFDM_AGC_TAB_0);
	else if (tab_sel == OFDM_AGC_TAB_2)
		odm_set_bb_reg(p_dm, BBREG_0xc70, 0x1e00, OFDM_AGC_TAB_2);
	else
		odm_set_bb_reg(p_dm, BBREG_0xc70, 0x1e00, OFDM_AGC_TAB_0);
}
...
)
{
	...
	return (u1Byte)odm_get_bb_reg(p_dm, BBREG_0xc70, 0x1e00);
}
	...
	)
{
	...
	//func_start = ODM_GetBBReg(pDM_Odm, BBREG_0x560, bMaskDWord);
	...
	odm_set_rf_reg(p_dm, RF_PATH_A, RFREG_0x86, 0x1f, 0x10);
	odm_set_rf_reg(p_dm, RF_PATH_B, RFREG_0x86, 0x1f, 0x10);

	// check saturation status
	for (i = 0; i < lna_sat_chk_cnt; i++) {
		sat_status_patha = odm_get_rf_reg(p_dm, RF_PATH_A, RFREG_0xae, 0xc0000);
		sat_status_pathb = odm_get_rf_reg(p_dm, RF_PATH_B, RFREG_0xae, 0xc0000);
	...
	//func_end = ODM_GetBBReg(pDM_Odm, BBREG_0x560, bMaskDWord);
	...
	)
{
	...
	if (state == INIT_LNA_SAT_CHK_TIMMER) {
		odm_initialize_timer(p_dm,
				     &p_lna_info->phydm_lna_sat_chk_timer,
...
void phydm_dig_debug(void *p_dm_void, char input[][16], u32 *_used,
		     char *output, u32 *_out_len, u32 input_num)
{
	...
	if ((strcmp(input[1], help) == 0))
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				...
				p_dig_t->fa_th[0], p_dig_t->fa_th[1],
				p_dig_t->fa_th[2]));

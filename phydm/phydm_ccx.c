...
u16 phydm_hw_divider(void *p_dm_void, u16 numerator, u16 denumerator)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		reg_devider_input = BBREG_0x980;
		reg_devider_rpt = BBREG_0x9f0;
	}
	...
	for (i = 0; i < 10; i++) {
		ODM_delay_ms(1);
		if (odm_get_bb_reg(p_dm, reg_devider_rpt,
				   BIT(24))) { /*Chk HW rpt is ready*/

			result = (u16)odm_get_bb_reg(p_dm, reg_devider_rpt,
						     MASKBYTE2);
			break;
		}
	}
...
void phydm_fahm_trigger(void *p_dm_void, u16 trigger_period /*unit (4us)*/
			)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		fahm_reg1 = BBREG_0x994;
	} else {

		odm_set_bb_reg(p_dm, BBREG_0x978, 0xff000000,
			       (trigger_period & 0xff));
		odm_set_bb_reg(p_dm, BBREG_0x97c, 0xff,
			       (trigger_period & 0xff00) >> 8);

		fahm_reg1 = BBREG_0x890;
	}
...
void phydm_fahm_set_valid_cnt(void *p_dm_void, u8 numerator_sel,
			      u8 denumerator_sel)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _CCX_INFO *ccx_info = &p_dm->dm_ccx_info;
...
void phydm_fahm_get_result(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		reg_rpt = BBREG_0x9d8;
		reg_rpt_2 = BBREG_0x9f0;
	}

	for (i = 0; i < 10; i++) {

		if (odm_get_bb_reg(p_dm, reg_rpt_2,
				   BIT(31))) { /*Chk HW rpt is ready*/
	...
	for (i = 0; i < 6; i++) {
		reg_val_tmp =
			odm_get_bb_reg(p_dm, reg_rpt + (i << 2), MASKDWORD);
		...
		fahm_rpt_cnt[i * 2 + 1] =
			(u16)((reg_val_tmp & MASKHWORD) >> 16);
	}

	for (i = 0; i < 12; i++) {
		fahm_rpt[i] = phydm_hw_divider(p_dm, fahm_rpt_cnt[i],
					       fahm_denumerator);
	}
...
void phydm_fahm_set_th_by_igi(void *p_dm_void, u8 igi)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _CCX_INFO *ccx_info = &p_dm->dm_ccx_info;
	u8 fahm_th[11];
	...
	u8 th_gap =
		2 * IGI_TO_NHM_TH_MULTIPLIER; /*beacuse unit is 0.5dB for FAHM*/
	u8 i;
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(p_dm, BBREG_0x1c38, 0xffffff00,
			       ((fahm_th[2] << 24) | (fahm_th[1] << 16) |
				(fahm_th[0] << 8)));
		odm_set_bb_reg(p_dm, BBREG_0x1c78, 0xffffff00,
			       ((fahm_th[5] << 24) | (fahm_th[4] << 16) |
				(fahm_th[3] << 8)));
		odm_set_bb_reg(p_dm, BBREG_0x1c7c, 0xffffff00,
			       ((fahm_th[7] << 24) | (fahm_th[6] << 16)));
		odm_set_bb_reg(p_dm, BBREG_0x1cb8, 0xffffff00,
			       ((fahm_th[10] << 24) | (fahm_th[9] << 16) |
				(fahm_th[8] << 8)));
	} else {
		odm_set_bb_reg(p_dm, BBREG_0x970, MASKDWORD,
			       ((fahm_th[3] << 24) | (fahm_th[2] << 16) |
				(fahm_th[1] << 8) | fahm_th[0]));
		odm_set_bb_reg(p_dm, BBREG_0x974, MASKDWORD,
			       ((fahm_th[7] << 24) | (fahm_th[6] << 16) |
				(fahm_th[5] << 8) | fahm_th[4]));
		odm_set_bb_reg(
			p_dm, BBREG_0x978, MASKDWORD,
			((fahm_th[10] << 16) | (fahm_th[9] << 8) | fahm_th[8]));
	}
}
...
void phydm_fahm_init(void *p_dm_void)
{
	...
	phydm_fahm_set_th_by_igi(p_dm, p_dm->dm_dig_table.cur_ig_value);
}
...
)
{
	...
	for (i = 0; i < 200; i++) {
		if (odm_get_bb_reg(p_dm, reg1, BIT(reg1_bit))) { /*Chk HW rpt is ready*/
			is_ready = true;
			break;
		}
		ODM_delay_ms(1);
...
{
	struct PHY_DM_STRUCT	*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _CCX_INFO			*p_ccx = &p_dm->dm_ccx_info;
	u32			value32;
	u8			i;

	PHYDM_DBG(p_dm, DBG_ENV_MNTR, ("[%s]===>\n", __FUNCTION__));

	if (phydm_nhm_check_rdy(p_dm) == false) {
		PHYDM_DBG(p_dm, DBG_ENV_MNTR, ("Get NHM report Fail\n"));
		return false;
	}
		
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		value32 = odm_read_4byte(p_dm, BBREG_0xfa8);
		odm_move_memory(p_dm, &p_ccx->nhm_result[0], &value32, 32);

		value32 = odm_read_4byte(p_dm, BBREG_0xfac);
		odm_move_memory(p_dm, &p_ccx->nhm_result[4], &value32, 32);

		value32 = odm_read_4byte(p_dm, BBREG_0xfb0);
		odm_move_memory(p_dm, &p_ccx->nhm_result[8], &value32, 32);

		/*Get NHM duration*/
		value32 = odm_read_4byte(p_dm, BBREG_0xfb4);
		p_ccx->nhm_duration = (u16)(value32 & MASKLWORD);
	} else {
		value32 = odm_read_4byte(p_dm, BBREG_0x8d8);
		odm_move_memory(p_dm, &p_ccx->nhm_result[0], &value32, 32);

		value32 = odm_read_4byte(p_dm, BBREG_0x8dc);
		odm_move_memory(p_dm, &p_ccx->nhm_result[4], &value32, 32);

		value32 = odm_get_bb_reg(p_dm, BBREG_0x8d0, 0xffff0000);
		odm_move_memory(p_dm, &p_ccx->nhm_result[8], &value32, 16);

		value32 = odm_read_4byte(p_dm, BBREG_0x8d4);
		odm_move_memory(p_dm, &p_ccx->nhm_result[10], (&value32 +16),
				16);
	...
	/* sum all nhm_result */
	p_ccx->nhm_result_total = 0;
	for (i = 0; i <= 11; i++)
		p_ccx->nhm_result_total += p_ccx->nhm_result[i];

	PHYDM_DBG(p_dm, DBG_ENV_MNTR, ("NHM_Rpt=(H->L)[%d %d %d %d (igi) %d %d %d %d %d %d %d %d]\n",
		p_ccx->nhm_result[11], p_ccx->nhm_result[10], p_ccx->nhm_result[9],
		p_ccx->nhm_result[8], p_ccx->nhm_result[7], p_ccx->nhm_result[6],
		p_ccx->nhm_result[5], p_ccx->nhm_result[4], p_ccx->nhm_result[3],
		p_ccx->nhm_result[2], p_ccx->nhm_result[1], p_ccx->nhm_result[0]));
...
)
{
	...
	pdm_set_reg(p_dm, reg2, MASKDWORD, BYTE_2_DWORD(p_ccx->nhm_th[3], p_ccx->nhm_th[2], p_ccx->nhm_th[1], p_ccx->nhm_th[0]));
	pdm_set_reg(p_dm, reg3, MASKDWORD, BYTE_2_DWORD(p_ccx->nhm_th[7], p_ccx->nhm_th[6], p_ccx->nhm_th[5], p_ccx->nhm_th[4]));
	pdm_set_reg(p_dm, reg4, MASKBYTE0, p_ccx->nhm_th[8]);
	pdm_set_reg(p_dm, reg1, 0xffff00, BYTE_2_DWORD(0, 0, p_ccx->nhm_th[10], p_ccx->nhm_th[9]));

	PHYDM_DBG(p_dm, DBG_ENV_MNTR, ("NHM_th[H->L]=[%d %d %d %d %d %d %d %d %d %d %d]\n",
	p_ccx->nhm_th[10], p_ccx->nhm_th[9], p_ccx->nhm_th[8],p_ccx->nhm_th[7], 
	p_ccx->nhm_th[6], p_ccx->nhm_th[5],p_ccx->nhm_th[4], p_ccx->nhm_th[3], 
	p_ccx->nhm_th[2], p_ccx->nhm_th[1], p_ccx->nhm_th[0]));
}

boolean
phydm_nhm_update_chk(
	void		*p_dm_void,
...
)
{
	...
	if (th_rule == IGI_UNIFORM_2DB) {
		if ((p_ccx->echo_igi == p_dm->dm_dig_table.cur_ig_value) || 
			(th_rule != p_ccx->th_rule)) {
			
			return false;	/*no need to update*/
		}
		
		p_ccx->echo_igi = p_dm->dm_dig_table.cur_ig_value;
		p_ccx->nhm_th[0] = (p_ccx->echo_igi - CCA_CAP) * IGI_TO_NHM_TH_MULTIPLIER;
		
		for (i = 1; i <= 10; i++)
			p_ccx->nhm_th[i] = p_ccx->nhm_th[0] + th_gap * i;
...
)
{
	...
	if ((ignore_tx != p_ccx->nhm_inexclude_txon) || (ignore_cca != p_ccx->nhm_inexclude_cca)) {
		PHYDM_DBG(p_dm, DBG_ENV_MNTR, ("ignore{tx, cca}={%d, %d}\n", ignore_tx, ignore_cca));
		pdm_set_reg(p_dm, reg1, BBREG_0x700, BIT_2_BYTE(0, ignore_tx, ignore_cca, 1));

		p_ccx->nhm_inexclude_txon = ignore_tx;
		p_ccx->nhm_inexclude_cca = ignore_cca;
	}
	
	/*Set NHM period*/
	if (period != p_ccx->nhm_period) {
		pdm_set_reg(p_dm, reg2, MASKHWORD, period);
		p_ccx->nhm_period = period;
	}

	if (phydm_nhm_update_chk(p_dm, th_rule)) {
		phydm_nhm_set_th_reg(p_dm);
	}
}
...
void phydm_set_nhm_th_by_igi(void *p_dm_void, u8 igi)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _CCX_INFO *ccx_info = &p_dm->dm_ccx_info;
	u8 th_gap = 2 * IGI_TO_NHM_TH_MULTIPLIER;
	...
	ccx_info->nhm_th[0] =
		(ccx_info->echo_igi - CCA_CAP) * IGI_TO_NHM_TH_MULTIPLIER;
	for (i = 1; i <= 10; i++)
...
void phydm_nhm_setting(void *p_dm_void, u8 nhm_setting)
{
	...
	if (nhm_setting == SET_NHM_SETTING) {
		PHYDM_DBG(
			...
			("NHM_th[H->L]=[0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x]\n",
			 ccx_info->nhm_th[10], ccx_info->nhm_th[9],
			 ccx_info->nhm_th[8], ccx_info->nhm_th[7],
			 ccx_info->nhm_th[6], ccx_info->nhm_th[5],
			 ccx_info->nhm_th[4], ccx_info->nhm_th[3],
			 ccx_info->nhm_th[2], ccx_info->nhm_th[1],
			 ccx_info->nhm_th[0]));
	}
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		if (nhm_setting == SET_NHM_SETTING) {
			...
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(9),
				       ccx_info->nhm_inexclude_cca);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(10),
				       ccx_info->nhm_inexclude_txon);

			/*Set NHM period*/
			odm_set_bb_reg(p_dm, ODM_REG_CCX_PERIOD_11AC, MASKHWORD,
				       ccx_info->nhm_period);

			/*Set NHM threshold*/ /*Unit: PWdB U(8,1)*/
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH3_TO_TH0_11AC,
			...
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH7_TO_TH4_11AC,
			...
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH8_11AC, MASKBYTE0,
				       ccx_info->nhm_th[8]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC,
				       MASKBYTE2, ccx_info->nhm_th[9]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC,
				       MASKBYTE3, ccx_info->nhm_th[10]);

			/*CCX EN*/
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(8),
				       CCX_EN);
			...
			ccx_info->nhm_period_restore = (u16)odm_get_bb_reg(
				p_dm, ODM_REG_CCX_PERIOD_11AC, MASKHWORD);

			/*Store NHM threshold*/
			ccx_info->nhm_th_restore[0] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE0);
			ccx_info->nhm_th_restore[1] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE1);
			ccx_info->nhm_th_restore[2] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE2);
			ccx_info->nhm_th_restore[3] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE3);
			ccx_info->nhm_th_restore[4] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE0);
			...
			ccx_info->nhm_th_restore[6] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE2);
			ccx_info->nhm_th_restore[7] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE3);
			ccx_info->nhm_th_restore[8] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH8_11AC, MASKBYTE0);
			ccx_info->nhm_th_restore[9] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH9_TH10_11AC, MASKBYTE2);
			ccx_info->nhm_th_restore[10] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH9_TH10_11AC, MASKBYTE3);
			...
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC,
				       MASKBYTE2, ccx_info->nhm_th_restore[9]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC,
				       MASKBYTE3, ccx_info->nhm_th_restore[10]);
		} else
			return;
	}
		...
		if (nhm_setting == SET_NHM_SETTING) {
			/*Set disable_ignore_cca, disable_ignore_txon*/
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11N, BIT(9),
				       ccx_info->nhm_inexclude_cca);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11N, BIT(10),
				       ccx_info->nhm_inexclude_txon);

			/*Set NHM period*/
			odm_set_bb_reg(p_dm, ODM_REG_CCX_PERIOD_11N, MASKHWORD,
				       ccx_info->nhm_period);

			/*Set NHM threshold*/
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH3_TO_TH0_11N,
				       MASKBYTE0, ccx_info->nhm_th[0]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH3_TO_TH0_11N,
				       MASKBYTE1, ccx_info->nhm_th[1]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH3_TO_TH0_11N,
			...
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH3_TO_TH0_11N,
				       MASKBYTE3, ccx_info->nhm_th[3]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH7_TO_TH4_11N,
				       MASKBYTE0, ccx_info->nhm_th[4]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH7_TO_TH4_11N,
				       MASKBYTE1, ccx_info->nhm_th[5]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH7_TO_TH4_11N,
			...
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH7_TO_TH4_11N,
				       MASKBYTE3, ccx_info->nhm_th[7]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH8_11N, MASKBYTE0,
				       ccx_info->nhm_th[8]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11N,
				       MASKBYTE2, ccx_info->nhm_th[9]);
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11N,
				       MASKBYTE3, ccx_info->nhm_th[10]);

			/*CCX EN*/
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11N, BIT(8),
				       CCX_EN);
		} else if (nhm_setting == STORE_NHM_SETTING) {
			/*Store pervious disable_ignore_cca, disable_ignore_txon*/
			ccx_info->nhm_inexclude_cca_restore =
				(enum nhm_inexclude_cca)odm_get_bb_reg(
					p_dm, ODM_REG_NHM_TH9_TH10_11N, BIT(9));
			ccx_info->nhm_inexclude_txon_restore =
				(enum nhm_inexclude_txon)odm_get_bb_reg(
					p_dm, ODM_REG_NHM_TH9_TH10_11N,
					BIT(10));

			/*Store pervious NHM period*/
			ccx_info->nhm_period_restore = (u16)odm_get_bb_reg(
				p_dm, ODM_REG_CCX_PERIOD_11N, MASKHWORD);

			/*Store NHM threshold*/
			ccx_info->nhm_th_restore[0] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH3_TO_TH0_11N, MASKBYTE0);
			ccx_info->nhm_th_restore[1] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH3_TO_TH0_11N, MASKBYTE1);
			ccx_info->nhm_th_restore[2] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH3_TO_TH0_11N, MASKBYTE2);
			ccx_info->nhm_th_restore[3] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH3_TO_TH0_11N, MASKBYTE3);
			ccx_info->nhm_th_restore[4] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH7_TO_TH4_11N, MASKBYTE0);
			ccx_info->nhm_th_restore[5] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH7_TO_TH4_11N, MASKBYTE1);
			ccx_info->nhm_th_restore[6] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH7_TO_TH4_11N, MASKBYTE2);
			ccx_info->nhm_th_restore[7] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH7_TO_TH4_11N, MASKBYTE3);
			ccx_info->nhm_th_restore[8] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH8_11N, MASKBYTE0);
			ccx_info->nhm_th_restore[9] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH9_TH10_11N, MASKBYTE2);
			ccx_info->nhm_th_restore[10] = (u8)odm_get_bb_reg(
				p_dm, ODM_REG_NHM_TH9_TH10_11N, MASKBYTE3);
...
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u32 nhm_reg1 = (p_dm->support_ic_type & ODM_IC_11AC_SERIES) ?
			       BBREG_0x994 :
			       BBREG_0x890;
	...
	/*Trigger NHM*/
	pdm_set_reg(p_dm, nhm_reg1, BIT(1), 0);
	pdm_set_reg(p_dm, nhm_reg1, BIT(1), 1);
}
...
void phydm_get_nhm_result(void *p_dm_void)
{
	...
	PHYDM_DBG(
		...
		("NHM_result=(H->L)[%d %d %d %d (igi) %d %d %d %d %d %d %d %d]\n",
		 ccx_info->nhm_result[11], ccx_info->nhm_result[10],
		 ccx_info->nhm_result[9], ccx_info->nhm_result[8],
		 ccx_info->nhm_result[7], ccx_info->nhm_result[6],
		 ccx_info->nhm_result[5], ccx_info->nhm_result[4],
		 ccx_info->nhm_result[3], ccx_info->nhm_result[2],
		 ccx_info->nhm_result[1], ccx_info->nhm_result[0]));
}

boolean phydm_check_nhm_rdy(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u8 i;
	boolean is_ready = false;
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		for (i = 0; i < 200; i++) {
			ODM_delay_ms(1);
			if (odm_get_bb_reg(p_dm, ODM_REG_NHM_DUR_READY_11AC,
					   BIT(16))) {
				is_ready = 1;
				break;
			}
		...
		for (i = 0; i < 200; i++) {
			ODM_delay_ms(1);
			if (odm_get_bb_reg(p_dm, BBREG_0x8b4, BIT(17))) {
				is_ready = 1;
				break;
			}
		}
...
void phydm_store_nhm_setting(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_nhm_init(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _CCX_INFO *ccx_info = &p_dm->dm_ccx_info;
		   ...
		   u32 *_out_len, u32 input_num)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		if ((var1[0] == 1) || (var1[0] == 2)) {
			ccx_info->echo_igi = (u8)odm_get_bb_reg(
				p_dm, BBREG_0xc50, MASKBYTE0);
			target_rssi = ccx_info->echo_igi - 10;
			ccx_info->nhm_th[0] = (target_rssi - 15 + 10) *
					      2; /*Unit: PWdB U(8,1)*/
...
void phydm_clm_h2c(void *p_dm_void, u16 obs_time, u8 fw_clm_en)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_clm_setting(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0x990, MASKLWORD,
			       ccx_info->clm_period); /*4us sample 1 time*/

	} else if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0x894, MASKLWORD,
			       ccx_info->clm_period);
	}

	PHYDM_DBG(p_dm, DBG_ENV_MNTR,
		  ("Set CLM period=%d * 4us\n", ccx_info->clm_period));
}

void phydm_clm_hw_restart(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;

	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_reg(p_dm, ODM_REG_CLM_11AC, BIT(8),
			       0x0); /*Enable CCX for CLM*/
		odm_set_bb_reg(p_dm, ODM_REG_CLM_11AC, BIT(8),
			       0x1); /*Enable CCX for CLM*/

	} else if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		odm_set_bb_reg(p_dm, ODM_REG_CLM_11N, BIT(8),
			       0x0); /*Enable CCX for CLM*/
		odm_set_bb_reg(p_dm, ODM_REG_CLM_11N, BIT(8),
			       0x1); /*Enable CCX for CLM*/
	}
...
void phydm_clm_trigger(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_reg(p_dm, ODM_REG_CLM_11AC, BIT(0),
			       0x0); /*Trigger CLM*/
		odm_set_bb_reg(p_dm, ODM_REG_CLM_11AC, BIT(0), 0x1);
	} else if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		odm_set_bb_reg(p_dm, ODM_REG_CLM_11N, BIT(0),
			       0x0); /*Trigger CLM*/
		odm_set_bb_reg(p_dm, ODM_REG_CLM_11N, BIT(0), 0x1);
	}
}

boolean phydm_clm_check_rdy(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	boolean is_ready = false;
	u8 i;
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		for (i = 0; i < 200; i++) {
			ODM_delay_ms(1);
			if (odm_get_bb_reg(p_dm, ODM_REG_CLM_RESULT_11AC,
					   BIT(16))) {
				is_ready = 1;
				break;
			}
		...
		for (i = 0; i < 200; i++) {
			ODM_delay_ms(1);
			if (odm_get_bb_reg(p_dm, ODM_REG_CLM_READY_11N,
					   BIT(16))) {
				is_ready = 1;
				break;
			}
...
void phydm_clm_get_result(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES)
		ccx_info->clm_result =
			(u16)odm_get_bb_reg(p_dm, BBREG_0xfa4, MASKDWORD);
	else if (p_dm->support_ic_type & ODM_IC_11N_SERIES)
		ccx_info->clm_result =
			(u16)odm_get_bb_reg(p_dm, BBREG_0x8d0, MASKDWORD);
...
void phydm_clm_init(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _CCX_INFO *ccx_info = &p_dm->dm_ccx_info;
...
void phydm_ccx_monitor_trigger(void *p_dm_void, u16 monitor_time /*unit ms*/
			       )
{
	u8 nhm_th[11], i, igi;
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		nhm_th[0] = (u8)odm_get_bb_reg(
			p_dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE0);
		nhm_th[1] = (u8)odm_get_bb_reg(
			p_dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE1);
		nhm_th[2] = (u8)odm_get_bb_reg(
			p_dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE2);
		nhm_th[3] = (u8)odm_get_bb_reg(
			p_dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE3);
		nhm_th[4] = (u8)odm_get_bb_reg(
			p_dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE0);
		nhm_th[5] = (u8)odm_get_bb_reg(
			p_dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE1);
		nhm_th[6] = (u8)odm_get_bb_reg(
			p_dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE2);
		nhm_th[7] = (u8)odm_get_bb_reg(
			p_dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE3);
		nhm_th[8] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH8_11AC,
					       MASKBYTE0);
		nhm_th[9] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC,
					       MASKBYTE2);
		nhm_th[10] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC,
						MASKBYTE3);
	} else if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {

		nhm_th[0] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH3_TO_TH0_11N,
					       MASKBYTE0);
		nhm_th[1] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH3_TO_TH0_11N,
					       MASKBYTE1);
		nhm_th[2] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH3_TO_TH0_11N,
					       MASKBYTE2);
		nhm_th[3] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH3_TO_TH0_11N,
					       MASKBYTE3);
		nhm_th[4] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH7_TO_TH4_11N,
					       MASKBYTE0);
		nhm_th[5] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH7_TO_TH4_11N,
					       MASKBYTE1);
		nhm_th[6] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH7_TO_TH4_11N,
					       MASKBYTE2);
		nhm_th[7] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH7_TO_TH4_11N,
					       MASKBYTE3);
		nhm_th[8] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH8_11N,
					       MASKBYTE0);
		nhm_th[9] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11N,
					       MASKBYTE2);
		nhm_th[10] = (u8)odm_get_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11N,
						MASKBYTE3);
	}
	...
	/*[NHM]*/
	igi = (u8)odm_get_bb_reg(p_dm, BBREG_0xc50, MASKBYTE0);
...
void phydm_ccx_monitor_result(void *p_dm_void)
{
	...
	if (phydm_check_nhm_rdy(p_dm)) {
		...
		if (ccx_info->nhm_result_total != 0)
			ccx_info->nhm_ratio =
				(u8)(((ccx_info->nhm_result_total -
				       ccx_info->nhm_result[0]) *
				      100) >>
				     8);
	}
	...
	if (ccx_info->clm_mntr_mode == CLM_DRIVER_MNTR) {
		...
		if (ccx_info->clm_period != 0) {
			if (ccx_info->clm_period == 64000)
				ccx_info->clm_ratio = (u8)(
					((ccx_info->clm_result >> 6) + 5) / 10);
			else if (ccx_info->clm_period == 65535) {
				clm_result_tmp =
					(u32)(ccx_info->clm_result * 100);
				ccx_info->clm_ratio = (u8)(
					(clm_result_tmp + (1 << 15)) >> 16);
			} else
				ccx_info->clm_ratio =
					(u8)((ccx_info->clm_result * 100) /
					     ccx_info->clm_period);
		}

	} else {
		if (ccx_info->clm_fw_result_cnt != 0)
			ccx_info->clm_ratio = (u8)(ccx_info->clm_fw_result_acc /
						   ccx_info->clm_fw_result_cnt);
		else
...
out:
	PHYDM_DBG(p_dm, DBG_ENV_MNTR,
		  ("IGI=0x%x, nhm_ratio=%d, clm_ratio=%d\n\n",
		   ccx_info->echo_igi, ccx_info->nhm_ratio,
		   ccx_info->clm_ratio));
}

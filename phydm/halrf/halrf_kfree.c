...
void phydm_set_kfree_to_rf_8814a(void *p_dm_void, u8 e_rf_path, u8 data)
{
	...
	if ((data % 2) != 0) { /*odd->positive*/
		data = data - 1;
		odm_set_rf_reg(p_dm, e_rf_path, REG_RF_TX_GAIN_OFFSET, BIT(19),
			       1);
		is_odd = true;
	} else { /*even->negative*/
		odm_set_rf_reg(p_dm, e_rf_path, REG_RF_TX_GAIN_OFFSET, BIT(19),
	...
	switch (data) {
	case 0:
		odm_set_rf_reg(p_dm, e_rf_path, REG_RF_TX_GAIN_OFFSET, BIT(14),
			       0);
		odm_set_rf_reg(p_dm, e_rf_path, REG_RF_TX_GAIN_OFFSET,
	...
	if (is_odd == false) {
		...
		p_rf_calibrate_info->kfree_offset[e_rf_path] =
			(-1) * p_rf_calibrate_info->kfree_offset[e_rf_path];
		ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
...
//
//
void phydm_get_thermal_trim_offset_8821c(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	odm_efuse_one_byte_read(p_dm, PPG_THERMAL_OFFSET_8821C, &pg_therm,
				false);
	...
	if (p_power_trim_info->flag & KFREE_FLAG_THERMAL_K_ON)
		ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
			     ("[kfree] 8821c thermal:%d\n",
			      p_power_trim_info->thermal));
}

void phydm_get_power_trim_offset_8821c(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_power_trim_data *p_power_trim_info = &p_dm->power_trim_data;
	...
	odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_2G_TXAB_OFFSET_8821C,
				&pg_power, false);

	if (pg_power != 0xff) {
		p_power_trim_info->bb_gain[0][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL1_TXA_OFFSET_8821C,
					&pg_power, false);
		p_power_trim_info->bb_gain[1][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL2_TXA_OFFSET_8821C,
					&pg_power, false);
		p_power_trim_info->bb_gain[2][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM1_TXA_OFFSET_8821C,
					&pg_power, false);
		p_power_trim_info->bb_gain[3][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM2_TXA_OFFSET_8821C,
					&pg_power, false);
		p_power_trim_info->bb_gain[4][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GH1_TXA_OFFSET_8821C,
					&pg_power, false);
		p_power_trim_info->bb_gain[5][0] = pg_power;
		p_power_trim_info->flag = p_power_trim_info->flag |
	...
	if (p_power_trim_info->flag & KFREE_FLAG_ON) {
		for (i = 0; i < KFREE_BAND_NUM; i++)
			ODM_RT_TRACE(
				p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
				("[kfree] 8821c power_trim_data->bb_gain[%d][0]=0x%X\n",
				 i, p_power_trim_info->bb_gain[i][0]));
	}
}

void phydm_set_kfree_to_rf_8821c(void *p_dm_void, u8 e_rf_path, boolean wlg_btg,
				 u8 data)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_rf_calibration_structure *p_rf_calibrate_info =
	...
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0xde, BIT(0), 1);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0xde, BIT(5), 1);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x55, BIT(6), 1);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x65, BIT(6), 1);

	if (wlg_btg == true) {
		wlg = data & 0xf;
		btg = (data & 0xf0) >> 4;

		odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x55, BIT(19),
			       (wlg & BIT(0)));
		odm_set_rf_reg(
			p_dm, e_rf_path, RFREG_0x55,
			(BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)),
			(wlg >> 1));

		odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x65, BIT(19),
			       (btg & BIT(0)));
		odm_set_rf_reg(
			p_dm, e_rf_path, RFREG_0x65,
			(BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)),
			(btg >> 1));
	} else {
		odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x55, BIT(19),
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
		     ("[kfree] 8821c 0x55[19:14]=0x%X 0x65[19:14]=0x%X\n",
		      odm_get_rf_reg(p_dm, e_rf_path, RFREG_0x55,
				     (BIT(19) | BIT(18) | BIT(17) | BIT(16) |
				      BIT(15) | BIT(14))),
		      odm_get_rf_reg(p_dm, e_rf_path, RFREG_0x65,
				     (BIT(19) | BIT(18) | BIT(17) | BIT(16) |
				      BIT(15) | BIT(14)))));
}

void phydm_clear_kfree_to_rf_8821c(void *p_dm_void, u8 e_rf_path, u8 data)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_rf_calibration_structure *p_rf_calibrate_info =
		&p_dm->rf_calibrate_info;
	...
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x55, BIT(19), (data & BIT(0)));
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x55,
		       (BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)),
		       (data >> 1));

	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x65, BIT(19), (data & BIT(0)));
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x65,
		       (BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)),
		       (data >> 1));

	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0xde, BIT(0), 0);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0xde, BIT(5), 0);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x55, BIT(6), 0);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x65, BIT(6), 0);
...
void phydm_get_thermal_trim_offset_8822b(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_power_trim_data *p_power_trim_info = &p_dm->power_trim_data;
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
		     ("[kfree] 8822b thermal trim flag:0x%02x\n",
		      p_power_trim_info->flag));

	if (p_power_trim_info->flag & KFREE_FLAG_THERMAL_K_ON)
		ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
			     ("[kfree] 8822b thermal:%d\n",
			      p_power_trim_info->thermal));
#if 0
	} else
		return;
...
void phydm_get_power_trim_offset_8822b(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_power_trim_data *p_power_trim_info = &p_dm->power_trim_data;
	struct odm_rf_calibration_structure *p_rf_calibrate_info =
	...
	odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_2G_TXAB_OFFSET, &pg_power,
				false);

	if (pg_power != 0xff) {
		/*Path A*/
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_2G_TXAB_OFFSET,
					&pg_power, false);
	...
	odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL1_TXA_OFFSET, &pg_power,
				false);

	if (pg_power != 0xff) {
		/*Path A*/
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL1_TXA_OFFSET,
					&pg_power, false);
		p_power_trim_info->bb_gain[1][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL2_TXA_OFFSET,
					&pg_power, false);
		p_power_trim_info->bb_gain[2][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM1_TXA_OFFSET,
					&pg_power, false);
		p_power_trim_info->bb_gain[3][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM2_TXA_OFFSET,
					&pg_power, false);
		p_power_trim_info->bb_gain[4][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GH1_TXA_OFFSET,
					&pg_power, false);
		p_power_trim_info->bb_gain[5][0] = pg_power;

		/*Path B*/
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL1_TXB_OFFSET,
					&pg_power, false);
		p_power_trim_info->bb_gain[1][1] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL2_TXB_OFFSET,
					&pg_power, false);
		p_power_trim_info->bb_gain[2][1] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM1_TXB_OFFSET,
					&pg_power, false);
		p_power_trim_info->bb_gain[3][1] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM2_TXB_OFFSET,
					&pg_power, false);
		p_power_trim_info->bb_gain[4][1] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GH1_TXB_OFFSET,
					&pg_power, false);
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
		     ("[kfree] 8822b power trim flag:0x%02x\n",
		      p_power_trim_info->flag));
...
void phydm_set_pa_bias_to_rf_8822b(void *p_dm_void, u8 e_rf_path, s8 tx_pa_bias)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_rf_calibration_structure *p_rf_calibrate_info =
		&p_dm->rf_calibrate_info;

	tx_pa_bias = tx_pa_bias +
		     (u8)odm_get_rf_reg(p_dm, e_rf_path, RFREG_0x51,
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
		     ("[kfree] 8822b 2g tx pa bias=0x%X rf 0x51=0x%X path=%d\n",
		      tx_pa_bias,
		      odm_get_rf_reg(p_dm, e_rf_path, RFREG_0x51,
				     (BIT(6) | BIT(5) | BIT(4) | BIT(3))),
		      e_rf_path));

	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0xef, BIT(10), 0x1);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x33, RFREGOFFSETMASK, 0x0);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x3f,
		       (BIT(12) | BIT(11) | BIT(10) | BIT(9)), tx_pa_bias);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x33, BIT(0), 0x1);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x3f,
		       (BIT(12) | BIT(11) | BIT(10) | BIT(9)), tx_pa_bias);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x33, BIT(1), 0x1);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x3f,
		       (BIT(12) | BIT(11) | BIT(10) | BIT(9)), tx_pa_bias);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x33, (BIT(1) | BIT(0)), 0x3);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x3f,
		       (BIT(12) | BIT(11) | BIT(10) | BIT(9)), tx_pa_bias);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0xef, BIT(10), 0x0);

	ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
		     ("[kfree] 8822b 2g tx pa bias rf_0x3f(0x%X) path=%d\n",
		      odm_get_rf_reg(p_dm, e_rf_path, RFREG_0x3f,
				     (BIT(12) | BIT(11) | BIT(10) | BIT(9))),
		      e_rf_path));
}

void phydm_get_pa_bias_offset_8822b(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_power_trim_data *p_power_trim_info = &p_dm->power_trim_data;
	...
	odm_efuse_one_byte_read(p_dm, PPG_PA_BIAS_2G_TXA_OFFSET, &pg_pa_bias,
				false);

	if (pg_pa_bias != 0xff) {
		/*paht a*/
		odm_efuse_one_byte_read(p_dm, PPG_PA_BIAS_2G_TXA_OFFSET,
					&pg_pa_bias, false);
		...
		/*paht b*/
		odm_efuse_one_byte_read(p_dm, PPG_PA_BIAS_2G_TXB_OFFSET,
					&pg_pa_bias, false);
		pg_pa_bias = pg_pa_bias & 0xf;
		...
		for (e_rf_path = RF_PATH_A; e_rf_path < 2; e_rf_path++)
			phydm_set_pa_bias_to_rf_8822b(p_dm, e_rf_path,
						      tx_pa_bias[e_rf_path]);
	} else
		ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
			     ("[kfree] 8822b 2g tx pa bias no pg\n"));
}

void phydm_set_kfree_to_rf_8822b(void *p_dm_void, u8 e_rf_path, u8 data)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_rf_calibration_structure *p_rf_calibrate_info =
		&p_dm->rf_calibrate_info;
	...
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0xde, BIT(0), 1);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0xde, BIT(4), 1);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x65, MASKLWORD, 0x9000);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x55, BIT(5), 1);
...
void phydm_clear_kfree_to_rf_8822b(void *p_dm_void, u8 e_rf_path, u8 data)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_rf_calibration_structure *p_rf_calibrate_info =
		&p_dm->rf_calibrate_info;
	...
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0xde, BIT(4), 1);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x65, MASKLWORD, 0x9000);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x55, BIT(5), 0);
	odm_set_rf_reg(p_dm, e_rf_path, RFREG_0x55, BIT(7), 0);
...
void phydm_set_kfree_to_rf(void *p_dm_void, u8 e_rf_path, u8 data)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_clear_kfree_to_rf(void *p_dm_void, u8 e_rf_path, u8 data)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_get_thermal_trim_offset(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_get_power_trim_offset(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_get_pa_bias_offset(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
s8 phydm_get_thermal_offset(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct odm_power_trim_data *p_power_trim_info = &p_dm->power_trim_data;
...
void phydm_config_kfree(void *p_dm_void, u8 channel_to_sw)
{
	...
	if (p_rf_calibrate_info->reg_rf_kfree_enable == 2) {
		...
		return;
	} else if (p_rf_calibrate_info->reg_rf_kfree_enable == 1 ||
		   p_rf_calibrate_info->reg_rf_kfree_enable == 0) {
		ODM_RT_TRACE(
			p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
		...
		if (*p_dm->p_band_type == ODM_BAND_2_4G &&
			...
			for (rfpath = RF_PATH_A; rfpath < max_rf_path;
			     rfpath++) {
				ODM_RT_TRACE(
					p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
					("[kfree] phydm_kfree(): channel_to_sw=%d PATH_%d bb_gain:0x%X\n",
					 channel_to_sw, rfpath,
					 p_power_trim_info->bb_gain[channel_idx]
								   [rfpath]));
				phydm_set_kfree_to_rf(
					p_dm, rfpath,
					p_power_trim_info
						->bb_gain[channel_idx][rfpath]);
			}

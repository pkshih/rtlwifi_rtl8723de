...
void phydm_get_read_counter(struct PHY_DM_STRUCT *p_dm)
{
	...
	while (1) {
		if ((odm_get_rf_reg(p_dm, RF_PATH_A, 0x8, RFREGOFFSETMASK) ==
		     0xabcde) ||
		    (counter > 300))
			break;
		counter++;
...
void do_iqk_8822b(void *p_dm_void, u8 delta_thermal_index, u8 thermal_value,
		  u8 threshold)
{
...
void _iqk_rf_set_check(struct PHY_DM_STRUCT *p_dm, u8 path, u16 add, u32 data)
{
	u32 i;
	...
	for (i = 0; i < 100; i++) {
		if (odm_get_rf_reg(p_dm, (enum rf_path)path, add,
				   RFREGOFFSETMASK) == data)
			break;
		else {
			ODM_delay_us(10);
			odm_set_rf_reg(p_dm, (enum rf_path)path, add,
				       RFREGOFFSETMASK, data);
		}
	}
}

void _iqk_rf0xb0_workaround(struct PHY_DM_STRUCT *p_dm)
{
	/*add 0xb8 control for the bad phase noise after switching channel*/
	odm_set_rf_reg(p_dm, (enum rf_path)0x0, RFREG_0xb8, RFREGOFFSETMASK,
		       0x00a00);
	odm_set_rf_reg(p_dm, (enum rf_path)0x0, RFREG_0xb8, RFREGOFFSETMASK,
		       0x80a00);
}

void _iqk_fill_iqk_report_8822b(void *p_dm_void, u8 channel)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	...
	for (i = 0; i < SS_8822B; i++) {
		tmp1 = tmp1 +
		       ((p_iqk_info->iqk_fail_report[channel][i][TX_IQK] & 0x1)
			<< i);
		tmp2 = tmp2 +
		       ((p_iqk_info->iqk_fail_report[channel][i][RX_IQK] & 0x1)
			<< (i + 4));
		tmp3 = tmp3 + ((p_iqk_info->rxiqk_fail_code[channel][i] & 0x3)
			       << (i * 2 + 8));
	}
	odm_write_4byte(p_dm, 0x1b00, 0xf8000008);
	odm_set_bb_reg(p_dm, BBREG_0x1bf0, 0x0000ffff, tmp1 | tmp2 | tmp3);

	for (i = 0; i < 2; i++)
		odm_write_4byte(
			p_dm, 0x1be8 + (i * 4),
			(p_iqk_info->rxiqk_agc[channel][(i * 2) + 1] << 16) |
				p_iqk_info->rxiqk_agc[channel][i * 2]);
}

void _iqk_fail_count_8822b(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u8 i;
	...
	p_dm->n_iqk_cnt++;
	if (odm_get_rf_reg(p_dm, RF_PATH_A, RFREG_0x1bf0, BIT(16)) == 1)
	...
	if (!p_iqk_info->is_reload) {
		for (i = 0; i < 8; i++) {
			if (odm_get_bb_reg(p_dm, BBREG_0x1bf0, BIT(i)) == 1)
				p_dm->n_iqk_fail_cnt++;
		}
	}
...
void _iqk_iqk_fail_report_8822b(struct PHY_DM_STRUCT *p_dm)
{
	u32 tmp1bf0 = 0x0;
	u8 i;
...
void _iqk_backup_mac_bb_8822b(struct PHY_DM_STRUCT *p_dm, u32 *MAC_backup,
...
void _iqk_backup_rf_8822b(struct PHY_DM_STRUCT *p_dm, u32 RF_backup[][2],
			  u32 *backup_rf_reg)
{
	u32 i;

	for (i = 0; i < RF_REG_NUM_8822B; i++) {
		RF_backup[i][RF_PATH_A] = odm_get_rf_reg(
			p_dm, RF_PATH_A, backup_rf_reg[i], RFREGOFFSETMASK);
		RF_backup[i][RF_PATH_B] = odm_get_rf_reg(
			p_dm, RF_PATH_B, backup_rf_reg[i], RFREGOFFSETMASK);
	}
	/*	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]BackupRF Success!!!!\n")); */
}

void _iqk_agc_bnd_int_8822b(struct PHY_DM_STRUCT *p_dm)
{
...
void _iqk_bb_reset_8822b(struct PHY_DM_STRUCT *p_dm)
{
	...
	/*reset BB report*/
	odm_set_bb_reg(p_dm, BBREG_0x8f8, 0x0ff00000, 0x0);

	while (1) {
		odm_write_4byte(p_dm, BBREG_0x8fc, 0x0);
		odm_set_bb_reg(p_dm, BBREG_0x198c, 0x7, 0x7);
		cca_ing = (boolean)odm_get_bb_reg(p_dm, BBREG_0xfa0, BIT(3));
		...
		if (cca_ing) {
			...
			odm_write_1byte(p_dm, BBREG_0x808, 0x0); /*RX ant off*/
			odm_set_bb_reg(p_dm, BBREG_0xa04,
			...
			if (odm_get_bb_reg(p_dm, BBREG_0x660, BIT(16)))
				odm_write_4byte(p_dm, BBREG_0x6b4, 0x89000006);
...
void _iqk_afe_setting_8822b(struct PHY_DM_STRUCT *p_dm, boolean do_iqk)
{
	if (do_iqk) {
		odm_write_4byte(p_dm, BBREG_0xc60, 0x50000000);
		odm_write_4byte(p_dm, BBREG_0xc60, 0x70070040);
		odm_write_4byte(p_dm, BBREG_0xe60, 0x50000000);
		odm_write_4byte(p_dm, BBREG_0xe60, 0x70070040);
		odm_write_4byte(p_dm, BBREG_0xc58, 0xd8000402);
		odm_write_4byte(p_dm, BBREG_0xc5c, 0xd1000120);
		odm_write_4byte(p_dm, BBREG_0xc6c, 0x00000a15);
		odm_write_4byte(p_dm, BBREG_0xe58, 0xd8000402);
		odm_write_4byte(p_dm, BBREG_0xe5c, 0xd1000120);
		odm_write_4byte(p_dm, BBREG_0xe6c, 0x00000a15);
		_iqk_bb_reset_8822b(p_dm);
		/*		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]AFE setting for IQK mode!!!!\n")); */
	} else {
		odm_write_4byte(p_dm, BBREG_0xc60, 0x50000000);
		odm_write_4byte(p_dm, BBREG_0xc60, 0x70038040);
		odm_write_4byte(p_dm, BBREG_0xe60, 0x50000000);
		odm_write_4byte(p_dm, BBREG_0xe60, 0x70038040);
		odm_write_4byte(p_dm, BBREG_0xc58, 0xd8020402);
		odm_write_4byte(p_dm, BBREG_0xc5c, 0xde000120);
		odm_write_4byte(p_dm, BBREG_0xc6c, 0x0000122a);
		odm_write_4byte(p_dm, BBREG_0xe58, 0xd8020402);
		odm_write_4byte(p_dm, BBREG_0xe5c, 0xde000120);
		odm_write_4byte(p_dm, BBREG_0xe6c, 0x0000122a);
		/*		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]AFE setting for Normal mode!!!!\n")); */
	}
	/*0x9a4[31]=0: Select da clock*/
	odm_set_bb_reg(p_dm, BBREG_0x9a4, BIT(31), 0x0);
}

void _iqk_restore_mac_bb_8822b(struct PHY_DM_STRUCT *p_dm, u32 *MAC_backup,
			       u32 *BB_backup, u32 *backup_mac_reg,
			       u32 *backup_bb_reg)
{
	u32 i;
...
void _iqk_restore_rf_8822b(struct PHY_DM_STRUCT *p_dm, u32 *backup_rf_reg,
			   u32 RF_backup[][2])
{
	u32 i;

	odm_set_rf_reg(p_dm, RF_PATH_A, RFREG_0xef, RFREGOFFSETMASK, 0x0);
	odm_set_rf_reg(p_dm, RF_PATH_B, RFREG_0xef, RFREGOFFSETMASK, 0x0);
	/*0xdf[4]=0*/
	_iqk_rf_set_check(p_dm, RF_PATH_A, 0xdf,
			  RF_backup[0][RF_PATH_A] & (~BIT(4)));
	_iqk_rf_set_check(p_dm, RF_PATH_B, 0xdf,
	...
	for (i = 1; i < RF_REG_NUM_8822B; i++) {
		odm_set_rf_reg(p_dm, RF_PATH_A, backup_rf_reg[i],
			       RFREGOFFSETMASK, RF_backup[i][RF_PATH_A]);
		odm_set_rf_reg(p_dm, RF_PATH_B, backup_rf_reg[i],
			       RFREGOFFSETMASK, RF_backup[i][RF_PATH_B]);
	}
	/*	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]RestoreRF Success!!!!\n")); */
}

void _iqk_backup_iqk_8822b(struct PHY_DM_STRUCT *p_dm, u8 step, u8 path)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u8 i, j, k;
	...
	case 0:
		...
		for (i = 0; i < 2; i++) {
			...
			p_iqk_info->rxiqk_agc[1][i] =
				p_iqk_info->rxiqk_agc[0][i];
			p_iqk_info->bypass_iqk[1][i] =
				p_iqk_info->bypass_iqk[0][i];
			p_iqk_info->rxiqk_fail_code[1][i] =
				p_iqk_info->rxiqk_fail_code[0][i];
			for (j = 0; j < 2; j++) {
				p_iqk_info->iqk_fail_report[1][i][j] =
					p_iqk_info->iqk_fail_report[0][i][j];
				for (k = 0; k < 8; k++) {
					p_iqk_info->iqk_cfir_real[1][i][j][k] =
						p_iqk_info->iqk_cfir_real[0][i]
									 [j][k];
					p_iqk_info->iqk_cfir_imag[1][i][j][k] =
						p_iqk_info->iqk_cfir_imag[0][i]
									 [j][k];
				}
			}
		}
		...
		break;
	case 1: /*LOK backup*/
		p_iqk_info->lok_idac[0][path] = odm_get_rf_reg(
			p_dm, (enum rf_path)path, RFREG_0x58, RFREGOFFSETMASK);
		break;
	case 2: /*TXIQK backup*/
	case 3: /*RXIQK backup*/
...
void _iqk_reload_iqk_setting_8822b(
	struct PHY_DM_STRUCT *p_dm, u8 channel,
	...
	)
{
	...
	for (path = 0; path < 2; path++) {
		if (reload_idx == 2) {
			...
			tmp = odm_get_rf_reg(p_dm, (enum rf_path)path,
					     RFREG_0xdf, RFREGOFFSETMASK) |
			      BIT(4);
			_iqk_rf_set_check(p_dm, (enum rf_path)path, 0xdf, tmp);
			odm_set_rf_reg(p_dm, (enum rf_path)path, RFREG_0x58,
				       RFREGOFFSETMASK,
				       p_iqk_info->lok_idac[channel][path]);
		}

		for (idx = 0; idx < reload_idx; idx++) {
			odm_set_bb_reg(p_dm, BBREG_0x1b00, MASKDWORD,
				       0xf8000008 | path << 1);
			odm_set_bb_reg(p_dm, BBREG_0x1b2c, MASKDWORD, 0x7);
			odm_set_bb_reg(p_dm, BBREG_0x1b38, MASKDWORD,
				       0x20000000);
			odm_set_bb_reg(p_dm, BBREG_0x1b3c, MASKDWORD,
				       0x20000000);
			odm_set_bb_reg(p_dm, BBREG_0x1bcc, MASKDWORD,
				       0x00000000);
			if (idx == 0)
				odm_set_bb_reg(p_dm, BBREG_0x1b0c,
					       BIT(13) | BIT(12), 0x3);
			else
				odm_set_bb_reg(p_dm, BBREG_0x1b0c,
					       BIT(13) | BIT(12), 0x1);
			odm_set_bb_reg(p_dm, BBREG_0x1bd4,
			...
			for (i = 0; i < 8; i++) {
				odm_write_4byte(
					...
					((0xc0000000 >> idx) + 0x3) + (i * 4) +
						(p_iqk_info->iqk_cfir_real
							 [channel][path][idx][i]
						 << 9));
				odm_write_4byte(
					p_dm, 0x1bd8,
					((0xc0000000 >> idx) + 0x1) + (i * 4) +
						(p_iqk_info->iqk_cfir_imag
							 [channel][path][idx][i]
						 << 9));
			}
			if (idx == 0)
				odm_set_bb_reg(p_dm, iqk_apply[path], BIT(0),
					       ~(p_iqk_info->iqk_fail_report
							 [channel][path][idx]));
			else
				odm_set_bb_reg(p_dm, iqk_apply[path], BIT(10),
					       ~(p_iqk_info->iqk_fail_report
							 [channel][path][idx]));
		}
		odm_set_bb_reg(p_dm, BBREG_0x1bd8, MASKDWORD, 0x0);
		odm_set_bb_reg(p_dm, BBREG_0x1b0c, BIT(13) | BIT(12), 0x0);
	}
}

boolean _iqk_reload_iqk_8822b(struct PHY_DM_STRUCT *p_dm, boolean reset)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u8 i;
	p_iqk_info->is_reload = false;
	...
	if (reset) {
		...
		p_iqk_info->rf_reg18 = odm_get_rf_reg(
			p_dm, RF_PATH_A, RFREG_0x18, RFREGOFFSETMASK);

		for (i = 0; i < 2; i++) {
			if (p_iqk_info->rf_reg18 ==
			    p_iqk_info->iqk_channel[i]) {
				_iqk_reload_iqk_setting_8822b(p_dm, i, 2);
				_iqk_fill_iqk_report_8822b(p_dm, i);
				ODM_RT_TRACE(
					p_dm, ODM_COMP_CALIBRATION,
					ODM_DBG_LOUD,
	...
	/*report*/
	odm_set_bb_reg(p_dm, BBREG_0x1bf0, BIT(16), (u8)p_iqk_info->is_reload);
	return p_iqk_info->is_reload;
}

void _iqk_rfe_setting_8822b(struct PHY_DM_STRUCT *p_dm, boolean ext_pa_on)
{
	if (ext_pa_on) {
		/*RFE setting*/
		odm_write_4byte(p_dm, BBREG_0xcb0, 0x77777777);
		odm_write_4byte(p_dm, BBREG_0xcb4, 0x00007777);
		odm_write_4byte(p_dm, BBREG_0xcbc, 0x0000083B);
		odm_write_4byte(p_dm, BBREG_0xeb0, 0x77777777);
		odm_write_4byte(p_dm, BBREG_0xeb4, 0x00007777);
		odm_write_4byte(p_dm, BBREG_0xebc, 0x0000083B);
		...
		odm_write_4byte(p_dm, BBREG_0xcb0, 0x77777777);
		odm_write_4byte(p_dm, BBREG_0xcb4, 0x00007777);
		odm_write_4byte(p_dm, BBREG_0xcbc, 0x00000100);
		odm_write_4byte(p_dm, BBREG_0xeb0, 0x77777777);
		odm_write_4byte(p_dm, BBREG_0xeb4, 0x00007777);
		odm_write_4byte(p_dm, BBREG_0xebc, 0x00000100);
		/*odm_write_4byte(p_dm, 0x1990, 0x00000c30);*/
		/*		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]external PA off!!!!\n"));*/
	}
}

void _iqk_rf_setting_8822b(struct PHY_DM_STRUCT *p_dm)
{
	u8 path;
	u32 tmp;
	...
	for (path = 0; path < 2; path++) {
		...
		tmp = odm_get_rf_reg(p_dm, (enum rf_path)path, RFREG_0xdf,
				     RFREGOFFSETMASK);
		...
		/*release 0x56 TXBB*/
		odm_set_rf_reg(p_dm, (enum rf_path)path, RFREG_0x65,
...
void _iqk_configure_macbb_8822b(struct PHY_DM_STRUCT *p_dm)
{
	/*MACBB register setting*/
	odm_write_1byte(p_dm, BBREG_0x522, 0x7f);
	odm_set_bb_reg(p_dm, BBREG_0x550, BIT(11) | BIT(3), 0x0);
	odm_set_bb_reg(p_dm, BBREG_0x90c, BIT(15),
		       0x1); /*0x90c[15]=1: dac_buf reset selection*/
	/*0xc94[0]=1, 0xe94[0]=1: 讓tx從iqk打出來*/
	odm_set_bb_reg(p_dm, BBREG_0xc94, BIT(0), 0x1);
	odm_set_bb_reg(p_dm, BBREG_0xe94, BIT(0), 0x1);
	odm_set_bb_reg(p_dm, BBREG_0xc94, (BIT(11) | BIT(10)), 0x1);
	odm_set_bb_reg(p_dm, BBREG_0xe94, (BIT(11) | BIT(10)), 0x1);
	/* 3-wire off*/
	odm_write_4byte(p_dm, BBREG_0xc00, 0x00000004);
	odm_write_4byte(p_dm, BBREG_0xe00, 0x00000004);
	/*disable PMAC*/
	odm_set_bb_reg(p_dm, BBREG_0xb00, BIT(8), 0x0);
	/*	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]Set MACBB setting for IQK!!!!\n"));*/
}

void _iqk_lok_setting_8822b(struct PHY_DM_STRUCT *p_dm,
			    ...
			    u8 path)
{
	...
	case ODM_BAND_2_4G:
		...
		odm_set_rf_reg(p_dm, (enum rf_path)path, RFREG_0x56,
			       RFREGOFFSETMASK, 0x50df2);
		odm_set_rf_reg(p_dm, (enum rf_path)path, RFREG_0x8f,
			       RFREGOFFSETMASK, 0xadc00);
		/* WE_LUT_TX_LOK*/
		odm_set_rf_reg(p_dm, (enum rf_path)path, RFREG_0xef, BIT(4),
			       0x1);
		odm_set_rf_reg(p_dm, (enum rf_path)path, RFREG_0x33,
			       BIT(1) | BIT(0), 0x0);
		break;
...
void _iqk_txk_setting_8822b(struct PHY_DM_STRUCT *p_dm, u8 path)
{
	odm_write_4byte(p_dm, 0x1b00, 0xf8000008 | path << 1);
	odm_write_4byte(p_dm, 0x1bcc, 0x9);
...
void _iqk_rxk1_setting_8822b(struct PHY_DM_STRUCT *p_dm, u8 path)
{
	odm_write_4byte(p_dm, 0x1b00, 0xf8000008 | path << 1);
...
void _iqk_rxk2_setting_8822b(struct PHY_DM_STRUCT *p_dm, u8 path, boolean is_gs)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
...
void halrf_iqk_set_rf0x8(struct PHY_DM_STRUCT *p_dm, u8 path)
{
	u16 c = 0x0;
	...
	while (c < 30000) {
		odm_set_rf_reg(p_dm, (enum rf_path)path, RFREG_0xef,
			       RFREGOFFSETMASK, 0x0);
		odm_set_rf_reg(p_dm, (enum rf_path)path, 0x8, RFREGOFFSETMASK,
			       0x0);
		if (odm_get_rf_reg(p_dm, (enum rf_path)path, 0x8,
				   RFREGOFFSETMASK) == 0x0)
			break;
		c++;
	}
...
void halrf_iqk_check_if_reload(struct PHY_DM_STRUCT *p_dm)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;

	p_iqk_info->is_reload =
		(boolean)odm_get_bb_reg(p_dm, BBREG_0x1bf0, BIT(16));
}

boolean _iqk_check_cal_8822b(struct PHY_DM_STRUCT *p_dm, u8 path, u8 cmd)
{
	...
	while (notready) {
		if (odm_get_rf_reg(p_dm, (enum rf_path)path, 0x8,
				   RFREGOFFSETMASK) == 0x12345) {
			if (cmd == 0x0) /*LOK*/
				fail = false;
			else
				fail = (boolean)odm_get_bb_reg(
					p_dm, BBREG_0x1b08, BIT(26));
			notready = false;
		} else {
			ODM_delay_ms(1);
			delay_count++;
...
boolean _iqk_rx_iqk_gain_search_fail_8822b(struct PHY_DM_STRUCT *p_dm, u8 path,
					   u8 step)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	boolean fail = true;
	...
	if (step == RXIQK2) {
		rf_reg0 = odm_get_rf_reg(p_dm, (enum rf_path)path, 0x0,
					 RFREGOFFSETMASK);
		odm_write_4byte(p_dm, 0x1b00, 0xf8000008 | path << 1);
		ODM_RT_TRACE(
			p_dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
			("[IQK]S%d ==> RF0x0 = 0x%x, tmp1bcc = 0x%x, idx = %d, 0x1b3c = 0x%x\n",
			 path, rf_reg0, p_iqk_info->tmp1bcc, idx,
			 odm_read_4byte(p_dm, 0x1b3c)));
		...
		if (fail) {
			...
			odm_write_4byte(
				p_dm, 0x1b24,
				(odm_read_4byte(p_dm, 0x1b24) & 0xffffe3ff) |
					(p_iqk_info->lna_idx << 10));
		}
	}
...
boolean _lok_one_shot_8822b(void *p_dm_void, u8 path)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	...
	if (ODM_COMP_CALIBRATION) {
		if (!LOK_notready) {
			LOK_temp = odm_get_rf_reg(p_dm, (enum rf_path)path,
						  RFREG_0x58, RFREGOFFSETMASK);
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
				     ("[IQK]0x58 = 0x%x\n", LOK_temp));
		} else
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
...
boolean _iqk_one_shot_8822b(void *p_dm_void, u8 path, u8 idx)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u8 delay_count = 0;
	...
	if (idx == TXIQK) {
		IQK_CMD = 0xf8000008 | ((*p_dm->p_band_width + 4) << 8) |
			  (1 << (path + 4));
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
		...
		IQK_CMD = 0xf8000008 | ((*p_dm->p_band_width + 9) << 8) |
			  (1 << (path + 4));
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
		...
		odm_write_4byte(p_dm, 0x1b00, 0xf8000008 | path << 1);
		odm_write_4byte(p_dm, 0x1b24,
				(odm_read_4byte(p_dm, 0x1b24) & 0xffffe3ff) |
					((p_iqk_info->lna_idx & 0x7) << 10));
	}
	odm_write_4byte(p_dm, 0x1b00, IQK_CMD);
	odm_write_4byte(p_dm, 0x1b00, IQK_CMD + 0x1);
	...
	if (p_dm->debug_components & ODM_COMP_CALIBRATION) {
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
			     ("[IQK]S%d ==> 0x1b00 = 0x%x, 0x1b08 = 0x%x\n",
			      path, odm_read_4byte(p_dm, 0x1b00),
		...
		if (idx != TXIQK)
			ODM_RT_TRACE(
				...
				("[IQK]S%d ==> RF0x0 = 0x%x, RF0x56 = 0x%x\n",
				 path, odm_get_rf_reg(p_dm, (enum rf_path)path,
						      0x0, RFREGOFFSETMASK),
				 odm_get_rf_reg(p_dm, (enum rf_path)path,
						RFREG_0x56, RFREGOFFSETMASK)));
	}
	...
	if (idx == RXIQK2) {
		p_iqk_info->rxiqk_agc[0][path] =
			(u16)(((odm_get_rf_reg(p_dm, (enum rf_path)path, 0x0,
					       RFREGOFFSETMASK) >>
				5) &
			       0xff) |
		...
		if (fail)
			odm_set_bb_reg(p_dm, iqk_apply[path],
				       (BIT(11) | BIT(10)), 0x0);
		else
...
boolean _iqk_rx_iqk_by_path_8822b(void *p_dm_void, u8 path)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	...
	case 1: /*gain search_RXK1*/
		...
		gonext = false;
		while (1) {
			KFAIL = _iqk_rx_iqk_gain_search_fail_8822b(p_dm, path,
								   RXIQK1);
			if (KFAIL &&
			    (p_iqk_info->gs_retry_count[0][path][0] < 2))
	...
	case 2: /*gain search_RXK2*/
		...
		while (1) {
			KFAIL = _iqk_rx_iqk_gain_search_fail_8822b(p_dm, path,
								   RXIQK2);
			if (KFAIL && (p_iqk_info->gs_retry_count[0][path][1] <
				      rxiqk_gs_limit))
	...
	case 3: /*RXK1*/
		...
		while (1) {
			...
			if (KFAIL &&
			    (p_iqk_info->retry_count[0][path][RXIQK1] < 2))
	...
	case 4: /*RXK2*/
		...
		while (1) {
			...
			if (KFAIL &&
			    (p_iqk_info->retry_count[0][path][RXIQK2] < 2))
...
void _iqk_iqk_by_path_8822b(void *p_dm_void, boolean segment_iqk)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	boolean KFAIL = true;
	...
	while (1) {
		...
		case 3: /*S0 TXIQK*/
			...
			if (KFAIL &&
			    (p_iqk_info->retry_count[0][RF_PATH_A][TXIQK] < 3))
		...
		case 4: /*S1 TXIQK*/
			...
			if (KFAIL &&
			    p_iqk_info->retry_count[0][RF_PATH_B][TXIQK] < 3)
		...
		case 5: /*S0 RXIQK*/
			while (1) {
				KFAIL = _iqk_rx_iqk_by_path_8822b(p_dm,
								  RF_PATH_A);
				ODM_RT_TRACE(
					p_dm, ODM_COMP_CALIBRATION,
					ODM_DBG_TRACE,
		...
		if (p_dm->rf_calibrate_info.iqk_step == 7) {
			...
			ODM_RT_TRACE(
				...
				("[IQK]PathA_TXIQK_fail = %d, PathB_TXIQK_fail = %d\n",
				 p_iqk_info
					 ->iqk_fail_report[0][RF_PATH_A][TXIQK],
				 p_iqk_info->iqk_fail_report[0][RF_PATH_B]
							    [TXIQK]));
			ODM_RT_TRACE(
				p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				("[IQK]PathA_RXIQK_fail = %d, PathB_RXIQK_fail = %d\n",
				 p_iqk_info
					 ->iqk_fail_report[0][RF_PATH_A][RXIQK],
				 p_iqk_info->iqk_fail_report[0][RF_PATH_B]
							    [RXIQK]));
			ODM_RT_TRACE(
				p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			...
			ODM_RT_TRACE(
				...
				("[IQK]PathA_RXK1_retry = %d, PathA_RXK2_retry = %d, PathB_RXK1_retry = %d, PathB_RXK2_retry = %d\n",
				 p_iqk_info->retry_count[0][RF_PATH_A][RXIQK1],
				 p_iqk_info->retry_count[0][RF_PATH_A][RXIQK2],
				 p_iqk_info->retry_count[0][RF_PATH_B][RXIQK1],
				 p_iqk_info
					 ->retry_count[0][RF_PATH_B][RXIQK2]));
			ODM_RT_TRACE(
				p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				("[IQK]PathA_GS1_retry = %d, PathA_GS2_retry = %d, PathB_GS1_retry = %d, PathB_GS2_retry = %d\n",
				 p_iqk_info
					 ->gs_retry_count[0][RF_PATH_A][RXIQK1],
				 p_iqk_info
					 ->gs_retry_count[0][RF_PATH_A][RXIQK2],
				 p_iqk_info
					 ->gs_retry_count[0][RF_PATH_B][RXIQK1],
				 p_iqk_info->gs_retry_count[0][RF_PATH_B]
							   [RXIQK2]));
			for (i = 0; i < 2; i++) {
		...
		if (segment_iqk && (p_iqk_info->kcount == kcount_limit))
			break;
#endif
	}
}

void _iqk_start_iqk_8822b(struct PHY_DM_STRUCT *p_dm, boolean segment_iqk)
{
	u32 tmp;
...
void _iq_calibrate_8822b_init(struct PHY_DM_STRUCT *p_dm)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u8 i, j, k, m;
	...
	if (firstrun) {
		...
		for (i = 0; i < 2; i++) {
			...
			for (j = 0; j < SS_8822B; j++) {
				...
				for (k = 0; k < 2; k++) {
					p_iqk_info->iqk_fail_report[i][j][k] =
						true;
					for (m = 0; m < 8; m++) {
						p_iqk_info
							->iqk_cfir_real[i][j][k]
...
void _phy_iq_calibrate_8822b(struct PHY_DM_STRUCT *p_dm, boolean reset,
			     boolean segment_iqk)
{
	u32 MAC_backup[MAC_REG_NUM_8822B], BB_backup[BB_REG_NUM_8822B],
		RF_backup[RF_REG_NUM_8822B][SS_8822B];
	u32 backup_mac_reg[MAC_REG_NUM_8822B] = {BBREG_0x520, BBREG_0x550};
	u32 backup_bb_reg[BB_REG_NUM_8822B] = {
		BBREG_0x808, BBREG_0x90c, BBREG_0xc00, BBREG_0xcb0, BBREG_0xcb4,
		BBREG_0xcbc, BBREG_0xe00, BBREG_0xeb0, BBREG_0xeb4, BBREG_0xebc,
		0x1990,      BBREG_0x9a4, BBREG_0xa04, BBREG_0xb00};
	...
	ODM_RT_TRACE(
		...
		("[IQK]p_band_type = %s, band_width = %d, ExtPA2G = %d, ext_pa_5g = %d\n",
		 (*p_dm->p_band_type == ODM_BAND_5G) ? "5G" : "2G",
		 *p_dm->p_band_width, p_dm->ext_pa, p_dm->ext_pa_5g));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
	...
	_iqk_backup_mac_bb_8822b(p_dm, MAC_backup, BB_backup, backup_mac_reg,
				 backup_bb_reg);
	...
	while (1) {
		...
		_iqk_restore_mac_bb_8822b(p_dm, MAC_backup, BB_backup,
...
void _phy_iq_calibrate_by_fw_8822b(void *p_dm_void, u8 clear, u8 segment_iqk)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;

...
void _iqk_page_switch(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_get_iqk_cfir(void *p_dm_void, u8 idx, u8 path, boolean debug)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	odm_set_bb_reg(p_dm, BBREG_0x1b00, MASKDWORD, 0xf8000008 | path << 1);
	if (idx == 0)
		odm_set_bb_reg(p_dm, BBREG_0x1b0c, BIT(13) | BIT(12), 0x3);
	else
		odm_set_bb_reg(p_dm, BBREG_0x1b0c, BIT(13) | BIT(12), 0x1);
	odm_set_bb_reg(p_dm, BBREG_0x1bd4,
		       BIT(20) | BIT(19) | BIT(18) | BIT(17) | BIT(16), 0x10);
	for (i = 0; i < 8; i++) {
		odm_set_bb_reg(p_dm, BBREG_0x1bd8, MASKDWORD,
			       0xe0000001 + (i * 4));
		tmp = odm_get_bb_reg(p_dm, BBREG_0x1bfc, MASKDWORD);
		p_iqk_info->iqk_cfir_real[ch][path][idx][i] =
			(tmp & 0x0fff0000) >> 16;
...
void halrf_iqk_xym_enable(struct PHY_DM_STRUCT *p_dm, u8 xym_enable)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	...
	PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION,
		  ("[IQK]%-20s %s\n", "xym_read = ",
		   (p_iqk_info->xym_read ? "true" : "false")));
}

void halrf_iqk_xym_read(
	void *p_dm_void, u8 path,
	...
	for (path = 0; path < 2; path++) {
		...
		case 0:
			for (i = 0; i < num; i++) {
				...
				p_iqk_info->rx_xym[path][i] =
					odm_read_4byte(p_dm, 0x1b38);
			}
			break;
		case 1:
			for (i = 0; i < num; i++) {
				...
				PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION,
					  ("[IQK]%-20s %-2d: 0x%x\n",
					   (path == 0) ? "PATH A RXK1-XYM " :
							 "PATH B RXK1-XYM",
					   i, p_iqk_info->rxk1_xym[path][i]));
			break;
		}
	}
}

void halrf_iqk_xym_dump(void *p_dm_void)
{
	u32 tmp1, tmp2;
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void halrf_iqk_info_dump(void *p_dm_void, u32 *_used, char *output,
			 u32 *_out_len)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u32 used = *_used;
...
void halrf_get_fw_version(void *p_dm_void)
{
...
#elif (DM_ODM_SUPPORT_TYPE == ODM_CE) && defined(DM_ODM_CE_MAC80211)
	{
		...
		p_rf->fw_ver =
			(rtlhal->fw_version << 16) | rtlhal->fw_subversion;
	}
#else
	{
		struct _ADAPTER		*adapter = p_dm->adapter;
...
void halrf_iqk_dbg(void *p_dm_void)
{
	...
	PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION,
		  ("%-20s: %s\n", "reload",
		   (p_iqk_info->is_reload) ? "True" : "False"));
...
*/
	...
	PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION,
		  ("%-20s: %llu %s\n", "progressing_time",
		   p_dm->rf_calibrate_info.iqk_progressing_time, "(ms)"));
...
void halrf_lck_dbg(struct PHY_DM_STRUCT *p_dm)
{
	...
	PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION,
		  ("%-20s: %llu %s\n", "progressing_time",
		   p_dm->rf_calibrate_info.lck_progressing_time, "(ms)"));
}

void halrf_iqk_dbg_cfir_backup(struct PHY_DM_STRUCT *p_dm)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u8 path, idx, i;
			...
			for (i = 0; i < 8; i++) {
				PHYDM_DBG(
					...
					("[IQK]%-7s %-3s CFIR_real: %-2d: 0x%x\n",
					 ...
					 (idx == 0) ? "TX" : "RX", i,
					 p_iqk_info->iqk_cfir_real[2][path][idx]
								  [i]));
			}
			for (i = 0; i < 8; i++) {
				PHYDM_DBG(
					...
					("[IQK]%-7s %-3s CFIR_img:%-2d: 0x%x\n",
					 (path == 0) ? "PATH A" : "PATH B",
					 (idx == 0) ? "TX" : "RX", i,
					 p_iqk_info->iqk_cfir_imag[2][path][idx]
								  [i]));
			}
		}
	}
...
void halrf_iqk_dbg_cfir_backup_update(struct PHY_DM_STRUCT *p_dm)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u8 i, path, idx;
...
void halrf_iqk_dbg_cfir_reload(struct PHY_DM_STRUCT *p_dm)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u8 i, path, idx;
...
void halrf_iqk_dbg_cfir_write(struct PHY_DM_STRUCT *p_dm, u8 type, u32 path,
			      u32 idx, u32 i, u32 data)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	if (type == 0)
...
void halrf_iqk_dbg_cfir_backup_show(struct PHY_DM_STRUCT *p_dm)
{
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u8 path, idx, i;
...
void halrf_do_imr_test(void *p_dm_void, u8 flag_imr_test)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void halrf_iqk_debug(void *p_dm_void, u32 *const dm_value, u32 *_used,
		     char *output, u32 *_out_len)
{
	...
	if (dm_value[0] == 0x0)
		...
		halrf_iqk_dbg_cfir_write(p_dm, 0, dm_value[1], dm_value[2],
					 dm_value[3], dm_value[4]);
	else if (dm_value[0] == 0x11)
		halrf_iqk_dbg_cfir_write(p_dm, 1, dm_value[1], dm_value[2],
					 dm_value[3], dm_value[4]);
...
void halrf_iqk_hwtx_check(void *p_dm_void, boolean is_check)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	u32 tmp_b04;
	...
	if (is_check)
		p_iqk_info->is_hwtx =
			(boolean)odm_get_bb_reg(p_dm, BBREG_0xb00, BIT(8));
	else {
		if (p_iqk_info->is_hwtx) {
			tmp_b04 = odm_read_4byte(p_dm, BBREG_0xb04);
			odm_set_bb_reg(p_dm, BBREG_0xb04, BIT(3) | BIT(2), 0x0);
			odm_write_4byte(p_dm, BBREG_0xb04, tmp_b04);
		}
	}
}

void halrf_segment_iqk_trigger(void *p_dm_void, boolean clear,
			       boolean segment_iqk)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _IQK_INFORMATION *p_iqk_info = &p_dm->IQK_info;
	...
	if ((p_dm->p_mp_mode != NULL) && (p_rf->p_is_con_tx != NULL) &&
	    (p_rf->p_is_single_tone != NULL) &&
	    (p_rf->p_is_carrier_suppresion != NULL))
		if (*p_dm->p_mp_mode &&
		    ((*p_rf->p_is_con_tx || *p_rf->p_is_single_tone ||
	...
	if (dm_value[0] == 100) {
		...
		PHYDM_SNPRINTF(
			(output + used, out_len - used,
			 "00. (( %s ))Power Tracking\n",
			 ((p_rf->rf_supportability & HAL_RF_TX_PWR_TRACK) ?
				  ("V") :
				  ("."))));
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "01. (( %s ))IQK\n",
			 ((p_rf->rf_supportability & HAL_RF_IQK) ? ("V") :
								   ("."))));
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "02. (( %s ))LCK\n",
			 ((p_rf->rf_supportability & HAL_RF_LCK) ? ("V") :
								   ("."))));
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "03. (( %s ))DPK\n",
...
void halrf_supportability_init_mp(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _hal_rf_ *p_rf = &p_dm->rf_table;
	...
	case ODM_RTL8822B:
		p_rf->rf_supportability =
			HAL_RF_TX_PWR_TRACK | HAL_RF_IQK | HAL_RF_LCK |
			/*HAL_RF_DPK				|*/
			0;
		break;
	#endif
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_INIT, ODM_DBG_LOUD,

...
void odm_clear_txpowertracking_state(void *p_dm_void)
{
	...
	p_rf_calibrate_info->bb_swing_idx_cck_base =
		p_rf_calibrate_info->default_cck_index;
	p_rf_calibrate_info->bb_swing_idx_cck =
		p_rf_calibrate_info->default_cck_index;
	p_dm->rf_calibrate_info.CCK_index = 0;

	for (p = RF_PATH_A; p < MAX_RF_PATH; ++p) {
		p_rf_calibrate_info->bb_swing_idx_ofdm_base[p] =
			p_rf_calibrate_info->default_ofdm_index;
		p_rf_calibrate_info->bb_swing_idx_ofdm[p] =
			p_rf_calibrate_info->default_ofdm_index;
		p_rf_calibrate_info->OFDM_index[p] =
			p_rf_calibrate_info->default_ofdm_index;
	...
	p_rf_calibrate_info->modify_tx_agc_flag_path_a =
		false; /*Initial at Modify Tx Scaling mode*/
	p_rf_calibrate_info->modify_tx_agc_flag_path_b =
		false; /*Initial at Modify Tx Scaling mode*/
	p_rf_calibrate_info->modify_tx_agc_flag_path_c =
		false; /*Initial at Modify Tx Scaling mode*/
	p_rf_calibrate_info->modify_tx_agc_flag_path_d =
		false; /*Initial at Modify Tx Scaling mode*/
	...
	p_rf_calibrate_info->modify_tx_agc_value_cck =
		0; /* modify by Mingzhi.Guo */
	p_rf_calibrate_info->modify_tx_agc_value_ofdm =
		0; /* modify by Mingzhi.Guo */
}
	...
	)
{
	...
	u8 indexforchannel =
		0; /* get_right_chnl_place_for_iqk(p_hal_data->current_channel) */
#if (DM_ODM_SUPPORT_TYPE == ODM_CE) && defined(DM_ODM_CE_MAC80211)
	...
	(*c.get_delta_swing_table)(p_dm, (u8 **)&delta_swing_table_idx_tup_a,
	...
	if (p_dm->support_ic_type & ODM_RTL8814A) /*for 8814 path C & D*/
		(*c.get_delta_swing_table8814only)(
			p_dm, (u8 **)&delta_swing_table_idx_tup_c,
	...
	if (p_dm->support_ic_type &
		...
		(*c.get_delta_swing_xtal_table)(
			p_dm, (s8 **)&delta_swing_table_xtal_up,
			(s8 **)&delta_swing_table_xtal_down);

	p_rf_calibrate_info
		->txpowertracking_callback_cnt++; /*cosa add for debug*/
...
/*p_rf_calibrate_info->txpowertrack_control = p_hal_data->txpowertrack_control;
	...
	ODM_RT_TRACE(
		...
		("===>odm_txpowertracking_callback_thermal_meter\n p_rf_calibrate_info->bb_swing_idx_cck_base: %d, p_rf_calibrate_info->bb_swing_idx_ofdm_base[A]: %d, p_rf_calibrate_info->default_ofdm_index: %d\n",
		 p_rf_calibrate_info->bb_swing_idx_cck_base,
		 p_rf_calibrate_info->bb_swing_idx_ofdm_base[RF_PATH_A],
		 p_rf_calibrate_info->default_ofdm_index));
	...
	thermal_value =
		(u8)odm_get_rf_reg(p_dm, RF_PATH_A, c.thermal_reg_addr,
	...
	if (p_dm->support_ic_type == ODM_RTL8723D) {
		regc80 = odm_get_bb_reg(p_dm, BBREG_0xc80, MASKDWORD);
		regcd0 = odm_get_bb_reg(p_dm, BBREG_0xcd0, MASKDWORD);
		regcd4 = odm_get_bb_reg(p_dm, BBREG_0xcd4, MASKDWORD);
		regab4 = odm_get_bb_reg(p_dm, BBREG_0xab4, 0x000007FF);
		ODM_RT_TRACE(
			p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			("BBREG_0xc80 = 0x%x BBREG_0xcd0 = 0x%x BBREG_0xcd4 = 0x%x BBREG_0xab4 = 0x%x\n",
	...
	if (rtlefu->eeprom_thermalmeter == 0xff) {
		ODM_RT_TRACE(
			p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			("no pg, p_hal_data->eeprom_thermal_meter = 0x%x\n",
			 rtlefu->eeprom_thermalmeter));
		return;
	}
#else
	...
	p_rf_calibrate_info->thermal_value_avg
		[p_rf_calibrate_info->thermal_value_avg_index] = thermal_value;
	p_rf_calibrate_info->thermal_value_avg_index++;
	if (p_rf_calibrate_info->thermal_value_avg_index ==
	...
	for (i = 0; i < c.average_thermal_num; i++) {
		if (p_rf_calibrate_info->thermal_value_avg[i]) {
			thermal_value_avg +=
				p_rf_calibrate_info->thermal_value_avg[i];
...
#if (DM_ODM_SUPPORT_TYPE == ODM_CE) && defined(DM_ODM_CE_MAC80211)
		p_rf_calibrate_info->thermal_value_delta =
	...
	delta = (thermal_value > p_rf_calibrate_info->thermal_value) ?
			(thermal_value - p_rf_calibrate_info->thermal_value) :
			(p_rf_calibrate_info->thermal_value - thermal_value);
	delta_LCK = (thermal_value > p_rf_calibrate_info->thermal_value_lck) ?
			    (thermal_value -
			     p_rf_calibrate_info->thermal_value_lck) :
			    (p_rf_calibrate_info->thermal_value_lck -
			     thermal_value);
	delta_IQK = (thermal_value > p_rf_calibrate_info->thermal_value_iqk) ?
			    (thermal_value -
			     p_rf_calibrate_info->thermal_value_iqk) :
			    (p_rf_calibrate_info->thermal_value_iqk -
			     thermal_value);

	if (p_rf_calibrate_info->thermal_value_iqk ==
	    0xff) { /*no PG, use thermal value for IQK*/
		p_rf_calibrate_info->thermal_value_iqk = thermal_value;
		delta_IQK = (thermal_value >
			     p_rf_calibrate_info->thermal_value_iqk) ?
				    (thermal_value -
				     p_rf_calibrate_info->thermal_value_iqk) :
				    (p_rf_calibrate_info->thermal_value_iqk -
	...
	for (p = RF_PATH_A; p < c.rf_path_count; p++)
		diff_DPK[p] = (s8)thermal_value -
			      (s8)p_rf_calibrate_info->dpk_thermal[p];
	...
	if (!(p_dm->support_ic_type &
		...
		if (p_rf_calibrate_info->thermal_value_lck == 0xff) {
			...
			if (!(p_dm->support_ic_type & ODM_RTL8814A) &&
			    !(p_dm->support_ic_type & ODM_RTL8822B) &&
			    c.phy_lc_calibrate)
				(*c.phy_lc_calibrate)(p_dm);

			delta_LCK = (thermal_value >
				     p_rf_calibrate_info->thermal_value_lck) ?
					    (thermal_value -
					     p_rf_calibrate_info
						     ->thermal_value_lck) :
		...
		if ((*p_dm->p_is_scan_in_process == false) &&
		    (!p_iqk_info->rfk_forbidden)) {
			/* Delta temperature is equal to or larger than 20 centigrade.*/
			if (delta_LCK >= c.threshold_iqk) {
				ODM_RT_TRACE(
					p_dm, ODM_COMP_TX_PWR_TRACK,
					ODM_DBG_LOUD,
...
/*4 7.1 The Final Power index = BaseIndex + power_index_offset*/
		...
		if (thermal_value > p_dm->priv->pmib->dot11RFEntry.ther) {
			...
			for (p = RF_PATH_A; p < c.rf_path_count; p++) {
				p_rf_calibrate_info->delta_power_index_last[p] =
					p_rf_calibrate_info->delta_power_index
				...
				case RF_PATH_B:
					...
					p_rf_calibrate_info
						...
						delta_swing_table_idx_tup_b
							[delta];
					p_rf_calibrate_info->absolute_ofdm_swing_idx
						[p] = delta_swing_table_idx_tup_b
						[delta]; /*Record delta swing for mix mode power tracking*/
					ODM_RT_TRACE(
						p_dm, ODM_COMP_TX_PWR_TRACK,
						ODM_DBG_LOUD,
			...
			if (p_dm->support_ic_type &
				...
				p_rf_calibrate_info->xtal_offset_last =
					p_rf_calibrate_info
						->xtal_offset; /*recording last Xtal offset*/
				ODM_RT_TRACE(
					p_dm, ODM_COMP_TX_PWR_TRACK,
					ODM_DBG_LOUD,
				...
				p_rf_calibrate_info->xtal_offset =
					delta_swing_table_xtal_up[delta];
				xtal_offset_eanble =
					(p_rf_calibrate_info
						 ->xtal_offset_last !=
		...
		for (p = RF_PATH_A; p < c.rf_path_count; p++) {
			...
			if (p_rf_calibrate_info->delta_power_index[p] ==
			    p_rf_calibrate_info->delta_power_index_last
				    [p]) /*If Thermal value changes but lookup table value still the same*/
				p_rf_calibrate_info->power_index_offset[p] = 0;
			else
				p_rf_calibrate_info->power_index_offset[p] =
					p_rf_calibrate_info
						->delta_power_index[p] -
					p_rf_calibrate_info->delta_power_index_last
			...
			ODM_RT_TRACE(
				...
				("[path-%d] power_index_offset(%d) = delta_power_index(%d) - delta_power_index_last(%d)\n",
				 p, p_rf_calibrate_info->power_index_offset[p],
				 p_rf_calibrate_info->delta_power_index[p],
				 p_rf_calibrate_info
					 ->delta_power_index_last[p]));

			p_rf_calibrate_info->OFDM_index[p] =
				p_rf_calibrate_info->bb_swing_idx_ofdm_base[p] +
				p_rf_calibrate_info->power_index_offset[p];
			p_rf_calibrate_info->CCK_index =
				p_rf_calibrate_info->bb_swing_idx_cck_base +
				p_rf_calibrate_info->power_index_offset[p];
			...
			ODM_RT_TRACE(
				...
				("The 'OFDM' final index(%d) = BaseIndex[%d](%d) + power_index_offset(%d)\n",
				 p_rf_calibrate_info->bb_swing_idx_ofdm[p], p,
				 p_rf_calibrate_info->bb_swing_idx_ofdm_base[p],
				 p_rf_calibrate_info->power_index_offset[p]));

			/*4 7.1 Handle boundary conditions of index.*/

			if (p_rf_calibrate_info->OFDM_index[p] >
			    c.swing_table_size_ofdm - 1)
				p_rf_calibrate_info->OFDM_index[p] =
					c.swing_table_size_ofdm - 1;
			else if (p_rf_calibrate_info->OFDM_index[p] <=
				 OFDM_min_index)
				p_rf_calibrate_info->OFDM_index[p] =
					OFDM_min_index;
		}
	...
	ODM_RT_TRACE(
		...
		("TxPowerTracking: [CCK] Swing Current index: %d, Swing base index: %d\n",
		 p_rf_calibrate_info->CCK_index,
		 p_rf_calibrate_info
			 ->bb_swing_idx_cck_base)); /*Print Swing base & current*/

	for (p = RF_PATH_A; p < c.rf_path_count; p++) {
		ODM_RT_TRACE(
			p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			("TxPowerTracking: [OFDM] Swing Current index: %d, Swing base index[%d]: %d\n",
			 p_rf_calibrate_info->OFDM_index[p], p,
			 p_rf_calibrate_info->bb_swing_idx_ofdm_base[p]));
	}
	...
	if ((p_dm->support_ic_type & ODM_RTL8814A)) {
		...
		if (power_tracking_type == 0) {
			...
			for (p = RF_PATH_A; p < c.rf_path_count; p++)
				(*c.odm_tx_pwr_track_set_pwr)(p_dm, MIX_MODE, p,
							      0);
			...
			for (p = RF_PATH_A; p < c.rf_path_count; p++)
				(*c.odm_tx_pwr_track_set_pwr)(
					p_dm, MIX_5G_TSSI_2G_MODE, p, 0);
		} else if (power_tracking_type == 3) {
			ODM_RT_TRACE(
				p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
				("**********Enter POWER Tracking TSSI MODE**********\n"));
			for (p = RF_PATH_A; p < c.rf_path_count; p++)
				(*c.odm_tx_pwr_track_set_pwr)(p_dm, TSSI_MODE,
							      p, 0);
		}
		p_rf_calibrate_info->thermal_value =
			thermal_value; /*Record last Power Tracking Thermal value*/
		...
		p_rf_calibrate_info->is_tx_power_changed =
...
#endif
		{
			...
			if (p_dm->support_ic_type == ODM_RTL8188E ||
			    ...
			    p_dm->support_ic_type == ODM_RTL8821 ||
			    p_dm->support_ic_type == ODM_RTL8812 ||
			    p_dm->support_ic_type == ODM_RTL8723B ||
			    p_dm->support_ic_type == ODM_RTL8814A ||
			    p_dm->support_ic_type == ODM_RTL8703B ||
			    p_dm->support_ic_type == ODM_RTL8188F ||
			    p_dm->support_ic_type == ODM_RTL8822B ||
			    p_dm->support_ic_type == ODM_RTL8723D ||
		...
		p_rf_calibrate_info->bb_swing_idx_cck_base =
			p_rf_calibrate_info
				->bb_swing_idx_cck; /*Record last time Power Tracking result as base.*/
		for (p = RF_PATH_A; p < c.rf_path_count; p++)
			p_rf_calibrate_info->bb_swing_idx_ofdm_base[p] =
				p_rf_calibrate_info->bb_swing_idx_ofdm[p];
	...
	if (p_dm->support_ic_type == ODM_RTL8703B ||
	    p_dm->support_ic_type == ODM_RTL8723D ||
	    p_dm->support_ic_type == ODM_RTL8710B) { /* JJ ADD 20161014 */
	...
	if (p_rf_calibrate_info->dpk_thermal[RF_PATH_A] != 0) {
		if (diff_DPK[RF_PATH_A] >= c.threshold_dpk) {
			odm_set_bb_reg(p_dm, BBREG_0x82c, BIT(31), 0x1);
			odm_set_bb_reg(p_dm, BBREG_0xcc4,
				       BIT(14) | BIT(13) | BIT(12) | BIT(11) |
					       BIT(10),
				       (diff_DPK[RF_PATH_A] / c.threshold_dpk));
			odm_set_bb_reg(p_dm, BBREG_0x82c, BIT(31), 0x0);
		} else if ((diff_DPK[RF_PATH_A] <= -1 * c.threshold_dpk)) {
			s32 value =
				0x20 + (diff_DPK[RF_PATH_A] / c.threshold_dpk);
	...
	if (p_rf_calibrate_info->dpk_thermal[RF_PATH_B] != 0) {
		if (diff_DPK[RF_PATH_B] >= c.threshold_dpk) {
			odm_set_bb_reg(p_dm, BBREG_0x82c, BIT(31), 0x1);
			odm_set_bb_reg(p_dm, BBREG_0xec4,
				       BIT(14) | BIT(13) | BIT(12) | BIT(11) |
					       BIT(10),
				       (diff_DPK[RF_PATH_B] / c.threshold_dpk));
			odm_set_bb_reg(p_dm, BBREG_0x82c, BIT(31), 0x0);
		} else if ((diff_DPK[RF_PATH_B] <= -1 * c.threshold_dpk)) {
			s32 value =
				0x20 + (diff_DPK[RF_PATH_B] / c.threshold_dpk);
...
void odm_reset_iqk_result(void *p_dm_void) { return; }
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)

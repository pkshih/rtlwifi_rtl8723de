...
boolean phydm_la_buffer_allocate(void *p_dm_void)
{
...
#else
		odm_allocate_memory(p_dm, (void **)&adc_smp_buf->octet,
...
void phydm_la_get_tx_pkt_buf(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
	struct _RT_ADCSMP_STRING *adc_smp_buf = &adc_smp->adc_smp_buf;
	...
	value32 = odm_read_4byte(p_dm, BBREG_0x7c0);
	is_round_up = (boolean)((value32 & BIT(31)) >> 31);
	finish_addr = (value32 & 0x7FFF0000) >>
		      16; /*Reg7C0[30:16]: finish addr (unit: 8byte)*/

	#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	#if (RTL8197F_SUPPORT)
	if (p_dm->support_ic_type & ODM_RTL8197F) {
		odm_set_bb_reg(p_dm, BBREG_0x7c0, BIT(0), 0x0);
		
		/*Stop DMA*/
		backup_dma = odm_get_mac_reg(p_dm, MACREG_0x300, MASKLWORD);
		odm_set_mac_reg(p_dm, MACREG_0x300, 0x7fff, 0x7fff);
	...
	if (is_round_up) {
		...
		dbg_print(
			"is_round_up = ((%d)), finish_addr=((0x%x)), BBREG_0x7c0=((0x%x))\n",
			is_round_up, finish_addr, value32);
		smp_number = ((adc_smp_buf->buffer_size) >>
			      3); /*Byte to 8Byte (64bit)*/
	} else {
	...
	*/
	...
	if (p_dm->support_ic_type & ODM_RTL8197F) {
		for (addr = 0x0, i = 0; addr < end_addr;
		     addr += 8, i += 2) { /*64K byte*/
			if ((addr & 0xfff) == 0)
				odm_set_bb_reg(p_dm, BBREG_0x0140, MASKLWORD,
					       0x780 + (addr >> 12));
			data_l = odm_get_bb_reg(
				p_dm, BBREG_0x8000 + (addr & 0xfff), MASKDWORD);
			data_h = odm_get_bb_reg(
				p_dm, BBREG_0x8000 + (addr & 0xfff) + 4,
				MASKDWORD);
	...
	#if (RTL8197F_SUPPORT)
	if (p_dm->support_ic_type & ODM_RTL8197F)
		odm_set_mac_reg(p_dm, MACREG_0x300, 0x7fff, backup_dma);	/*Resume DMA*/
	#endif
	#endif
}

void phydm_la_mode_set_mac_iq_dump(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
	u32 reg_value;

	odm_write_1byte(p_dm, BBREG_0x7c0, 0); /*clear all 0x7c0*/
	odm_set_mac_reg(p_dm, MACREG_0x7c0, BIT(0),
			1); /*Enable LA mode HW block*/

	if (adc_smp->la_trig_mode == PHYDM_MAC_TRIG) {
		adc_smp->is_bb_trigger = 0;
		odm_set_mac_reg(p_dm, MACREG_0x7c0, BIT(2),
				1); /*polling bit for MAC mode*/
		odm_set_mac_reg(
			p_dm, MACREG_0x7c0, BIT(4) | BIT(3),
		...
		dbg_print(
			...
			adc_smp->la_trig_sig_sel, adc_smp->la_dbg_port);
		/*[Set MAC Debug Port]*/
		odm_set_mac_reg(p_dm, 0xF4, BIT(16), 1);
		odm_set_mac_reg(p_dm, 0x38, 0xff0000, adc_smp->la_dbg_port);
		odm_set_mac_reg(p_dm, MACREG_0x7c4, MASKDWORD,
				adc_smp->la_mac_mask_or_hdr_sel);
		odm_set_mac_reg(p_dm, MACREG_0x7c8, MASKDWORD,
				adc_smp->la_trig_sig_sel);

	} else {
		adc_smp->is_bb_trigger = 1;
		odm_set_mac_reg(p_dm, MACREG_0x7c0, BIT(1),
				1); /*polling bit for BB ADC mode*/

		if (adc_smp->la_trig_mode == PHYDM_ADC_MAC_TRIG) {
			odm_set_mac_reg(
				p_dm, MACREG_0x7c0, BIT(3),
				1); /*polling bit for MAC trigger event*/
			odm_set_mac_reg(p_dm, MACREG_0x7c0, BIT(7) | BIT(6),
					adc_smp->la_trig_sig_sel);

			if (adc_smp->la_trig_sig_sel == ADCSMP_TRIG_REG)
				odm_set_mac_reg(
					p_dm, MACREG_0x7c0, BIT(5),
					1); /* manual trigger 0x7c0[5] = 0->1*/
		}
	}

	reg_value = odm_get_bb_reg(p_dm, BBREG_0x7c0, 0xff);
	dbg_print("4. [Set MAC IQ dump] BBREG_0x7c0[7:0] = ((0x%x))\n",
		  reg_value);
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	RT_TRACE_EX(COMP_LA_MODE, DBG_LOUD, ("4. [Set MAC IQ dump] BBREG_0x7c0[7:0] = ((0x%x))\n", reg_value));
#endif
}

void phydm_adc_smp_start(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
	u8 tmp_u1b;
	...
	if (p_dm->support_ic_type & ODM_RTL8197F)
		odm_set_bb_reg(p_dm, BBREG_0xd00, BIT(26), 0x1);
	else { /*for 8814A and 8822B?*/
		odm_write_1byte(p_dm, BBREG_0x8b4, 0x80);
	...
	do { /*Polling time always use 100ms, when it exceed 2s, break while loop*/
		tmp_u1b = odm_read_1byte(p_dm, BBREG_0x7c0);
	...
	if (adc_smp->la_count == 0) {
		...
		if ((p_dm->support_ic_type & ODM_RTL8821C) &&
		    (p_dm->cut_version >= ODM_CUT_B))
			odm_set_bb_reg(p_dm, BBREG_0x95c, BIT(23), 0);
...
void adc_smp_set(void *p_dm_void, u8 trig_mode, u32 trig_sig_sel,
...
void adc_smp_query(void *p_dm_void, void *output, u32 out_len, u32 *pused)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
	struct _RT_ADCSMP_STRING *adc_smp_buf = &adc_smp->adc_smp_buf;
...
s32 adc_smp_get_sample_counts(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
	struct _RT_ADCSMP_STRING *adc_smp_buf = &adc_smp->adc_smp_buf;
...
s32 adc_smp_query_single_data(void *p_dm_void, void *output, u32 out_len,
			      u32 index)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
...
void adc_smp_stop(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
...
void adc_smp_init(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
	struct _RT_ADCSMP_STRING *adc_smp_buf = &adc_smp->adc_smp_buf;
...
#if (DM_ODM_SUPPORT_TYPE & (ODM_WIN | ODM_CE))
void adc_smp_de_init(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
	struct _RT_ADCSMP_STRING *adc_smp_buf = &adc_smp->adc_smp_buf;
...
void phydm_la_mode_bb_setting(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_ADCSMP *adc_smp = &p_dm->adcsmp;
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(p_dm, BBREG_0x95c, 0xf00,
			       la_dma_type); /*0x95c[11:8]*/
		odm_set_bb_reg(p_dm, BBREG_0x95c, 0x1f,
			       trig_sig_sel); /*0x95c[4:0], BB debug port bit*/
		odm_set_bb_reg(p_dm, BBREG_0x95c, BIT(31),
			       is_trigger_edge); /*0: posedge, 1: negedge*/
		odm_set_bb_reg(p_dm, BBREG_0x95c, 0xe0, sampling_rate);
		/*	(0:) '80MHz'
			(1:) '40MHz'
			(2:) '20MHz'
			(3:) '10MHz'
		...
		*/
		if ((p_dm->support_ic_type & ODM_RTL8821C) &&
		    (p_dm->cut_version >= ODM_CUT_B)) {
			odm_set_bb_reg(p_dm, BBREG_0x95c, BIT(23), 1);
		}
	} else {
		odm_set_bb_reg(p_dm, BBREG_0x9a0, 0xf00,
			       la_dma_type); /*0x9a0[11:8]*/
		odm_set_bb_reg(p_dm, BBREG_0x9a0, 0x1f,
			       trig_sig_sel); /*0x9a0[4:0], BB debug port bit*/
		odm_set_bb_reg(p_dm, BBREG_0x9a0, BIT(31),
			       is_trigger_edge); /*0: posedge, 1: negedge*/
		odm_set_bb_reg(p_dm, BBREG_0x9a0, 0xe0, sampling_rate);
		/*	(0:) '80MHz'
			(1:) '40MHz'
			(2:) '20MHz'
			(3:) '10MHz'
...
void phydm_la_mode_set_trigger_time(void *p_dm_void, u32 trigger_time_mu_sec)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	dbg_print(
		"2. [Set Trigger Time] Trig_Time = ((%d)) * unit = ((2^%d us))\n",
		trigger_time_unit_num, time_unit);
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	RT_TRACE_EX(COMP_LA_MODE, DBG_LOUD, ("3. [Set Trigger Time] Trig_Time = ((%d)) * unit = ((2^%d us))\n", trigger_time_unit_num, time_unit));
#endif

	odm_set_mac_reg(p_dm, MACREG_0x7cc, BIT(20) | BIT(19) | BIT(18),
			time_unit);
	odm_set_mac_reg(p_dm, MACREG_0x7c0, 0x7f00,
			(trigger_time_unit_num & 0x7f));
}

void phydm_lamode_trigger_setting(void *p_dm_void, char input[][16], u32 *_used,
				  char *output, u32 *_out_len, u32 input_num)
{
	...
	if (p_dm->support_ic_type & PHYDM_IC_SUPPORT_LA_MODE) {
		...
		if ((strcmp(input[1], help) == 0)) {
			...
			adc_smp_set(p_dm, trig_mode, trig_sig_sel,

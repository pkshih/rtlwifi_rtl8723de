...
void phydm_write_cck_cca_th_new_cs_ratio(void *p_dm_void, u8 cca_th,
					 u8 cca_th_aaa)
{
	...
	PHYDM_DBG(p_dm, DBG_CCKPD,
		  ("[New] pd_th=0x%x, cs_ratio=0x%x\n\n", cca_th, cca_th_aaa));

	if (p_cckpd_t->cur_cck_cca_thres != cca_th) {

		p_cckpd_t->cur_cck_cca_thres = cca_th;
		odm_set_bb_reg(p_dm, BBREG_0xa08, 0xf0000, cca_th);
	...
	if (p_cckpd_t->cck_cca_th_aaa != cca_th_aaa) {
		...
		odm_set_bb_reg(p_dm, BBREG_0xaa8, 0x1f0000, cca_th_aaa);
		p_cckpd_t->cck_fa_ma = CCK_FA_MA_RESET;
	}
}

void phydm_write_cck_cca_th(void *p_dm_void, u8 cca_th)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_cckpd_struct *p_cckpd_t = &p_dm->dm_cckpd_table;
...
void phydm_set_cckpd_val(void *p_dm_void, u32 *val_buf, u8 val_len)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;

	if (val_len != 2) {
		PHYDM_DBG(p_dm, ODM_COMP_API,
			  ("[Error][CCKPD]Need val_len=2\n"));
		return;
	}
	...
	if (p_dm->support_ic_type & EXTEND_CCK_CCATH_AAA_IC) {
		phydm_write_cck_cca_th_new_cs_ratio(p_dm, (u8)val_buf[0],
						    (u8)val_buf[1]);
	} else {
...
boolean phydm_stop_cck_pd_th(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_cckpd_new_cs_ratio(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_dig_struct *p_dig_t = &p_dm->dm_dig_table;
	struct phydm_cckpd_struct *p_cckpd_t = &p_dm->dm_cckpd_table;
	...
	en_2rcca = (u8)(odm_get_bb_reg(p_dm, BBREG_0xa2c, BIT(18)) &&
			odm_get_bb_reg(p_dm, BBREG_0xa2c, BIT(22)));

	if (p_dm->is_linked) {

		if ((igi_curr > 0x38) && (p_dm->rssi_min > 32)) {
			cs_ration =
				p_dig_t->aaa_default + AAA_BASE + AAA_STEP * 2;
			cs_2r_offset = 5;
			pd_th = 0xd;
			...
			pd_th = 0xd;
			   ...
			   (p_dm->rssi_min > 24 && p_dm->rssi_min <= 30)) {
	...
	if (en_2rcca)
		cs_ration = (cs_ration >= cs_2r_offset) ?
				    (cs_ration - cs_2r_offset) :
				    0;
	...
	odm_set_bb_reg(p_dm, BBREG_0xa08, 0xf0000, pd_th);
	odm_set_bb_reg(p_dm, BBREG_0xaa8, 0x1f0000, cs_ration);
...
void phydm_cck_pd_th(void *p_dm_void)
{
#ifdef PHYDM_SUPPORT_CCKPD
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_cckpd_t->cck_fa_ma == CCK_FA_MA_RESET)
		...
		p_cckpd_t->cck_fa_ma =
			((p_cckpd_t->cck_fa_ma << 1) + p_cckpd_t->cck_fa_ma +
			 cnt_cck_fail_tmp) >>
			2;
	}
...
void odm_pause_cck_packet_detection(void *p_dm_void,
				    ...
				    u8 cck_pd_th)
{
	...
	PHYDM_DBG(p_dm, DBG_CCKPD,
		  ("Set pause{Type, LV, val} = {%d, %d, 0x%x}\n", pause_type,
		   pause_lv, cck_pd_th));
	...
	case PHYDM_PAUSE: {
		...
		p_cckpd_t->pause_bitmap |=
			BIT(pause_lv); /* Update pause level */
	...
	case PHYDM_RESUME: {
		...
		phydm_write_cck_cca_th(p_dm,
				       p_cckpd_t->pause_cckpd_value[max_level]);
		PHYDM_DBG(p_dm, DBG_CCKPD,
			  ("Write CCKPD=0x%x for max_LV=%d\n",
...
void phydm_cck_pd_init(void *p_dm_void)
{
#ifdef PHYDM_SUPPORT_CCKPD
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_cckpd_struct *p_cckpd_t = &p_dm->dm_cckpd_table;
	...
	if (p_dm->support_ic_type & EXTEND_CCK_CCATH_AAA_IC)
		p_dig_t->aaa_default = odm_read_1byte(p_dm, BBREG_0xaaa) & 0x1f;

	odm_memory_set(p_dm, p_cckpd_t->pause_cckpd_value, 0,
		       PHYDM_PAUSE_MAX_NUM);
#endif
}

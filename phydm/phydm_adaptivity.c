...
void phydm_dig_up_bound_lmt_en(void *p_dm_void)
{
	...
	if (!(p_dm->support_ability & ODM_BB_ADAPTIVITY) ||
	    !p_dm->adaptivity_flag || (!p_dm->is_linked) ||
	...
	if (p_dm->total_tp > 1) {
		...
		p_adaptivity->igi_up_bound_lmt_cnt =
			p_adaptivity->igi_up_bound_lmt_val;
...
void phydm_set_edcca_threshold(void *p_dm_void, s8 H2L, s8 L2H)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;

	if (p_dm->support_ic_type & ODM_IC_11N_SERIES)
		odm_set_bb_reg(p_dm, REG_OFDM_0_ECCA_THRESHOLD,
			       MASKBYTE2 | MASKBYTE0,
			       (u32)((u8)L2H | (u8)H2L << 16));
#if (RTL8195A_SUPPORT == 0)
	else if (p_dm->support_ic_type & ODM_IC_11AC_SERIES)
		odm_set_bb_reg(p_dm, REG_FPGA0_XB_LSSI_READ_BACK, MASKLWORD,
			       (u16)((u8)L2H | (u8)H2L << 8));
#endif
}

void phydm_set_lna(void *p_dm_void, enum phydm_set_lna type)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;

	if (p_dm->support_ic_type & (ODM_RTL8188E | ODM_RTL8192E)) {
		if (type == phydm_disable_lna) {
			odm_set_rf_reg(p_dm, RF_PATH_A, RFREG_0xef, 0x80000,
				       0x1);
			odm_set_rf_reg(p_dm, RF_PATH_A, RFREG_0x30, 0xfffff,
				       0x18000); /*select Rx mode*/
			odm_set_rf_reg(p_dm, RF_PATH_A, RFREG_0x31, 0xfffff,
				       0x0000f);
			odm_set_rf_reg(p_dm, RF_PATH_A, RFREG_0x32, 0xfffff,
				       0x37f82); /*disable LNA*/
			odm_set_rf_reg(p_dm, RF_PATH_A, RFREG_0xef, 0x80000,
				       0x0);
			if (p_dm->rf_type > RF_1T1R) {
				odm_set_rf_reg(p_dm, RF_PATH_B, RFREG_0xef,
					       0x80000, 0x1);
				odm_set_rf_reg(p_dm, RF_PATH_B, RFREG_0x30,
					       0xfffff, 0x18000);
				odm_set_rf_reg(p_dm, RF_PATH_B, RFREG_0x31,
					       0xfffff, 0x0000f);
				odm_set_rf_reg(p_dm, RF_PATH_B, RFREG_0x32,
					       0xfffff, 0x37f82);
				odm_set_rf_reg(p_dm, RF_PATH_B, RFREG_0xef,
					       0x80000, 0x0);
			}
		...
		if (type == phydm_disable_lna) {
			...
			odm_set_rf_reg(p_dm, RF_PATH_A, RFREG_0xef, 0x80000,
				       0x0);
			/*S1*/
			odm_set_rf_reg(p_dm, RF_PATH_A, RFREG_0xed, 0x00020,
				       0x1);
			odm_set_rf_reg(
				p_dm, RF_PATH_A, RFREG_0x43, 0xfffff,
...
void phydm_set_trx_mux(void *p_dm_void, enum phydm_trx_mux_type tx_mode,
		       enum phydm_trx_mux_type rx_mode)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		odm_set_bb_reg(
			p_dm, ODM_REG_CCK_RPT_FORMAT_11N,
			BIT(3) | BIT(2) | BIT(1),
			tx_mode); /*set TXmod to standby mode to remove outside noise affect*/
		odm_set_bb_reg(
			p_dm, ODM_REG_CCK_RPT_FORMAT_11N,
			BIT(22) | BIT(21) | BIT(20),
			rx_mode); /*set RXmod to standby mode to remove outside noise affect*/
		if (p_dm->rf_type > RF_1T1R) {
			odm_set_bb_reg(
				p_dm, ODM_REG_CCK_RPT_FORMAT_11N_B,
				BIT(3) | BIT(2) | BIT(1),
				tx_mode); /*set TXmod to standby mode to remove outside noise affect*/
			odm_set_bb_reg(
				p_dm, ODM_REG_CCK_RPT_FORMAT_11N_B,
...
void phydm_mac_edcca_state(void *p_dm_void, enum phydm_mac_edcca_type state)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	if (state == phydm_ignore_edcca) {
		odm_set_mac_reg(p_dm, REG_TX_PTCL_CTRL, BIT(15),
				1); /*ignore EDCCA	reg520[15]=1*/
		/*		odm_set_mac_reg(p_dm, REG_RD_CTRL, BIT(11), 0);			*/ /*reg524[11]=0*/
	} else { /*don't set MAC ignore EDCCA signal*/
		odm_set_mac_reg(p_dm, REG_TX_PTCL_CTRL, BIT(15),
				0); /*don't ignore EDCCA	 reg520[15]=0*/
		/*		odm_set_mac_reg(p_dm, REG_RD_CTRL, BIT(11), 1);			*/ /*reg524[11]=1	*/
	}
	PHYDM_DBG(p_dm, DBG_ADPTVTY, ("EDCCA enable state = %d\n", state));
}

void phydm_check_environment(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_adaptivity_struct *adaptivity =
		(struct phydm_adaptivity_struct *)phydm_get_structure(
...
void phydm_search_pwdb_lower_bound(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_adaptivity_struct *adaptivity =
		(struct phydm_adaptivity_struct *)phydm_get_structure(
	...
	s8 diff;
	u8 IGI = adaptivity->igi_base + 30 + (u8)p_dm->th_l2h_ini -
		 (u8)p_dm->th_edcca_hl_diff;

	if (p_dm->support_ic_type &
	    (ODM_RTL8723B | ODM_RTL8188E | ODM_RTL8192E | ODM_RTL8812 |
...
boolean phydm_re_search_condition(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_adaptivity_struct *adaptivity =
		(struct phydm_adaptivity_struct *)phydm_get_structure(
...
void phydm_adaptivity_info_init(void *p_dm_void, enum phydm_adapinfo_e cmn_info,
				u32 value)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_adaptivity_struct *adaptivity =
...
void phydm_adaptivity_init(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_adaptivity_struct *adaptivity =
		(struct phydm_adaptivity_struct *)phydm_get_structure(
...
/*struct phydm_dig_struct* p_dig_t = &p_dm->dm_dig_table;*/
...
#endif
		p_dm->edcca_enable =
			false; /*even no adaptivity, we still enable EDCCA, AP side use mib control*/
	else
	...
	adaptivity->adaptivity_dbg_port =
		(p_dm->support_ic_type & ODM_IC_11N_SERIES) ? BBREG_0x208 :
							      BBREG_0x209;
	...
	if (p_dm->support_ic_type & ODM_IC_11N_GAIN_IDX_EDCCA) {
		...
		if (p_dm->support_ic_type & ODM_RTL8197F) {
			odm_set_bb_reg(p_dm, ODM_REG_PAGE_B1_97F, BIT(30),
				       0x1); /*set to page B1*/
			odm_set_bb_reg(
				p_dm, ODM_REG_EDCCA_DCNF_97F, BIT(27) | BIT(26),
				0x1); /*0:rx_dfir, 1: dcnf_out, 2 :rx_iq, 3: rx_nbi_nf_out*/
			odm_set_bb_reg(p_dm, ODM_REG_PAGE_B1_97F, BIT(30), 0x0);
		} else
			odm_set_bb_reg(
				p_dm, ODM_REG_EDCCA_DCNF_11N, BIT(21) | BIT(20),
				0x1); /*0:rx_dfir, 1: dcnf_out, 2 :rx_iq, 3: rx_nbi_nf_out*/
	}
#if (RTL8195A_SUPPORT == 0)
	if (p_dm->support_ic_type &
	    ODM_IC_11AC_GAIN_IDX_EDCCA) { /*8814a no need to find pwdB lower bound, maybe*/
		/*odm_set_bb_reg(p_dm, ODM_REG_EDCCA_DOWN_OPT, BIT(30) | BIT(29) | BIT(28), 0x7);*/ /*interfernce need > 2^x us, and then EDCCA will be 1*/
		odm_set_bb_reg(
			p_dm, ODM_REG_ACBB_EDCCA_ENHANCE, BIT(29) | BIT(28),
			0x1); /*0:rx_dfir, 1: dcnf_out, 2 :rx_iq, 3: rx_nbi_nf_out*/
	}
	...
	if (!(p_dm->support_ic_type &
		...
		phydm_set_edcca_threshold(p_dm, 0x7f,
					  0x7f); /*resume to no link state*/
#endif
	...
	adaptivity->adajust_igi_level =
		(u8)((p_dm->th_l2h_ini + igi_target) - pwdb_upper_bound +
	...
	p_dm->adaptivity_flag =
		(p_dm->support_ic_type & ODM_IC_GAIN_IDX_EDCCA) ? false : true;
...
void phydm_adaptivity(void *p_dm_void)
{
	...
	if ((!(p_dm->support_ability & ODM_BB_ADAPTIVITY)) &&
		...
		if (p_dm->p_soft_ap_mode != NULL) {
			...
			PHYDM_DBG(p_dm, DBG_ADPTVTY,
				  ("p_dm->soft_ap_special_setting = %x, *(p_dm->p_soft_ap_mode) = %d, disable_ap_adapt_setting = %d\n", p_dm->soft_ap_special_setting, *p_dm->p_soft_ap_mode, disable_ap_adapt_setting));
		}
		...
		p_dm->th_edcca_hl_diff = 7;
		adaptivity->adajust_igi_level = (u8)(
			(p_dm->th_l2h_ini + igi_target) - pwdb_upper_bound +
			dfir_loss); /*IGI = L2H - PwdB - dfir_loss*/
	}
	PHYDM_DBG(p_dm, DBG_ADPTVTY, ("odm_Adaptivity() =====>\n"));
	PHYDM_DBG(p_dm, DBG_ADPTVTY,
		  ("igi_base=0x%x, th_l2h_ini = %d, th_edcca_hl_diff = %d\n",
		   adaptivity->igi_base, p_dm->th_l2h_ini,
		   p_dm->th_edcca_hl_diff));
#if (RTL8195A_SUPPORT == 0)
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		/*fix AC series when enable EDCCA hang issue*/
		odm_set_bb_reg(p_dm, BBREG_0x800, BIT(10),
			       1); /*ADC_mask disable*/
		odm_set_bb_reg(p_dm, BBREG_0x800, BIT(10),
			       0); /*ADC_mask enable*/
	}
	...
	PHYDM_DBG(
		p_dm, DBG_ADPTVTY,
		("band_width=%s, igi_target=0x%x, dynamic_link_adaptivity = %d\n",
		 (*p_dm->p_band_width == CHANNEL_WIDTH_80) ?
			 "80M" :
			 ((*p_dm->p_band_width == CHANNEL_WIDTH_40) ? "40M" :
								      "20M"),
		 igi_target, adaptivity->dynamic_link_adaptivity));
	PHYDM_DBG(
		p_dm, DBG_ADPTVTY,
		("adajust_igi_level= 0x%x, adaptivity_flag = %d, adaptivity_enable = %d\n",
		 adaptivity->adajust_igi_level, p_dm->adaptivity_flag,
		 p_dm->adaptivity_enable));

	if (adaptivity->dynamic_link_adaptivity && (!p_dm->is_linked) &&
	    !p_dm->adaptivity_enable) {
	...
	PHYDM_DBG(p_dm, DBG_ADPTVTY,
		  ("adapt_igi_up=0x%x, h2l_lb = 0x%x, l2h_lb = 0x%x\n",
		   adaptivity->adapt_igi_up, adaptivity->h2l_lb,
		   adaptivity->l2h_lb));
	PHYDM_DBG(p_dm, DBG_ADPTVTY,
...
void phydm_pause_edcca(void *p_dm_void, boolean is_pasue_edcca)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_adaptivity_struct *adaptivity =
		(struct phydm_adaptivity_struct *)phydm_get_structure(
...
void phydm_set_edcca_threshold_api(void *p_dm_void, u8 IGI)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_adaptivity_struct *adaptivity =
		(struct phydm_adaptivity_struct *)phydm_get_structure(
	...
	if (p_dm->support_ability & ODM_BB_ADAPTIVITY) {
		...
		PHYDM_DBG(
			p_dm, DBG_ADPTVTY,
			("API :adapt_igi_up=0x%x, h2l_lb = 0x%x, l2h_lb = 0x%x\n",
			 adaptivity->adapt_igi_up, adaptivity->h2l_lb,
			 adaptivity->l2h_lb));
...
void phydm_adaptivity_debug(void *p_dm_void, u32 *const dm_value, u32 *_used,
			    char *output, u32 *_out_len)
{
	...
	if (dm_value[0] == PHYDM_ADAPT_DEBUG) {
		...
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "EDCCA Resume\n"));
		p_dm->edcca_enable = true;
		...
		if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
			reg_value32 =
				odm_get_bb_reg(p_dm, BBREG_0xc4c, MASKDWORD);
			...
			reg_value32 =
				odm_get_bb_reg(p_dm, BBREG_0x8a4, MASKDWORD);
			h2l_diff = (s8)(0x000000ff & reg_value32) -
...
void phydm_set_l2h_th_ini(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_set_forgetting_factor(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;

	if (p_dm->support_ic_type &
	    (ODM_RTL8821C | ODM_RTL8822B | ODM_RTL8814A))
		odm_set_bb_reg(p_dm, BBREG_0x8a0, BIT(1) | BIT(0), 0);
}

void phydm_set_pwdb_mode(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;

	if (p_dm->support_ability & ODM_BB_ADAPTIVITY) {
		if (p_dm->support_ic_type & ODM_RTL8822B)
			odm_set_bb_reg(p_dm, BBREG_0x8dc, BIT(5), 0x1);
		else if (p_dm->support_ic_type & ODM_RTL8197F)
			odm_set_bb_reg(p_dm, BBREG_0xce8, BIT(13), 0x1);
	} else {
		if (p_dm->support_ic_type & ODM_RTL8822B)
			odm_set_bb_reg(p_dm, BBREG_0x8dc, BIT(5), 0x0);
		else if (p_dm->support_ic_type & ODM_RTL8197F)
			odm_set_bb_reg(p_dm, BBREG_0xce8, BIT(13), 0x0);
	}
}

void phydm_set_edcca_val(void *p_dm_void, u32 *val_buf, u8 val_len)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;

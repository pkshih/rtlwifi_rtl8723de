...
struct _odm_phy_dbg_info_ {
	...
	u16 num_qry_ht_pkt_20m[LOW_BW_RATE_NUM]; /*for 20M MCS 0 & 1*/
	u8 ht_pkt_not_zero;
	boolean ht_low_bw_occur;
	#if	ODM_IC_11AC_SERIES_SUPPORT
	u16 num_qry_vht_pkt[VHT_RATE_NUM];
	u16 num_qry_vht_pkt_20m[LOW_BW_RATE_NUM]; /*for 20M MCS 0 & 1*/
	u16 num_qry_vht_pkt_40m[LOW_BW_RATE_NUM]; /*for 40M MCS 0 & 1*/
	u8 vht_pkt_not_zero;
	boolean vht_low_bw_occur;
	#endif
...
#endif
{
	...
	struct sta_info *p_odm_sta_info
		[ODM_ASSOCIATE_ENTRY_NUM]; /*_ODM_STA_INFO, 2012/01/12 MH For MP, we need to reduce one array pointer for default port.??*/
	...
	struct timer_list
		cck_path_diversity_timer; /*2011.09.27 add for path Diversity*/
...
void phydm_fwoffload_ability_init(struct PHY_DM_STRUCT *p_dm,
				  enum phydm_offload_ability offload_ability);

void phydm_fwoffload_ability_clear(struct PHY_DM_STRUCT *p_dm,
				   enum phydm_offload_ability offload_ability);

void phydm_support_ability_debug(void *p_dm_void, u32 *const dm_value,
...
u8 phydm_pause_func(void *p_dm_void, enum phydm_func_idx_e pause_func,
		    enum phydm_pause_type pause_type,
		    enum phydm_pause_level pause_lv, u8 val_lehgth, u32 *val_buf

		    );

void phydm_pause_func_console(void *p_dm_void, char input[][16], u32 *_used,
			      char *output, u32 *_out_len, u32 input_num);

void odm_cmn_info_init(struct PHY_DM_STRUCT *p_dm, enum odm_cmninfo_e cmn_info,
		       u64 value);

void odm_cmn_info_hook(struct PHY_DM_STRUCT *p_dm, enum odm_cmninfo_e cmn_info,
		       void *p_value);

void odm_cmn_info_update(struct PHY_DM_STRUCT *p_dm, u32 cmn_info, u64 value);

u32 phydm_cmn_info_query(struct PHY_DM_STRUCT *p_dm,
...
void odm_release_all_timers(struct PHY_DM_STRUCT *p_dm);
...
void phydm_dc_cancellation(struct PHY_DM_STRUCT *p_dm);

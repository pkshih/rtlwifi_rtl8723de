...
void phydm_h2C_debug(void *p_dm_void, u32 *const dm_value, u32 *_used,
...
void phydm_fw_fix_rate(void *p_dm_void, u8 en, u8 macid, u8 bw, u8 rate
		       ...
		       )
{
	...
	if (p_dm->support_ic_type & PHYDM_IC_8051_SERIES) {
		...
		odm_set_bb_reg(p_dm, BBREG_0x4a0, MASKDWORD, reg_u32_tmp);

	} else {

		if (en == 1)
			reg_u32_tmp =
				(0x60 << 24) | (macid << 16) | (bw << 8) | rate;
		else
			reg_u32_tmp = 0x40000000;

		odm_set_bb_reg(p_dm, BBREG_0x450, MASKDWORD, reg_u32_tmp);
	}
	if (en == 1) {
...
void phydm_ra_debug(void *p_dm_void, char input[][16], u32 *_used, char *output,
		    u32 *_out_len)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if ((strcmp(input[1], help) == 0)) {
			...
			PHYDM_SNPRINTF(
				(output + used, out_len - used,
				 ...
				 ((p_ra_table->RA_threshold_offset == 0) ?
					  " " :
					  ((p_ra_table->RA_offset_direction) ?
						   "+" :
						   "-")),
		...
		phydm_fw_fix_rate(p_dm, (u8)var1[1], (u8)var1[2], (u8)var1[3],
				  (u8)var1[4]);
...
void odm_c2h_ra_para_report_handler(void *p_dm_void, u8 *cmd_buf, u8 cmd_len)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_ra_dynamic_retry_count(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_dm->pre_b_noisy != p_dm->noisy_decision) {
		if (p_dm->noisy_decision) {
			PHYDM_DBG(p_dm, DBG_DYN_ARFR,
				  ("Noisy Env. RA fallback\n"));
			odm_set_mac_reg(p_dm, MACREG_0x430, MASKDWORD, 0x0);
			odm_set_mac_reg(p_dm, MACREG_0x434, MASKDWORD,
					0x04030201);
		} else {
			PHYDM_DBG(p_dm, DBG_DYN_ARFR,
				  ("Clean Env. RA fallback\n"));
			odm_set_mac_reg(p_dm, MACREG_0x430, MASKDWORD,
					0x01000000);
			odm_set_mac_reg(p_dm, MACREG_0x434, MASKDWORD,
					0x06050402);
		}
...
void phydm_print_rate(void *p_dm_void, u8 rate, u32 dbg_component)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	PHYDM_DBG_F(p_dm, dbg_component,
...
void phydm_c2h_ra_report_handler(void *p_dm_void, u8 *cmd_buf, u8 cmd_len)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _rate_adaptive_table_ *p_ra_table = &p_dm->dm_ra_table;
	u8 macid = cmd_buf[1];
	...
	if (cmd_len >= 7) {
		...
		curr_bw = cmd_buf[6];
		PHYDM_DBG(p_dm, DBG_RA,
			  ("RA retry ratio: [%d]:", curr_ra_ratio));
		/**/
	}
	...
	if (is_sta_active(p_sta)) {
		...
		p_sta->ra_info.curr_tx_bw = (enum channel_width)curr_bw;
		p_sta->ra_info.curr_retry_ratio = curr_ra_ratio;

		/*if (p_sta->ra_info.curr_tx_bw <  p_sta->ra_info.ra_bw_mode)*/
	}
...
/*trigger power training*/
	...
	if ((p_dm->is_one_entry_only) ||
	    ((rate_order > p_ra_table->highest_client_tx_order) &&
...
void odm_ra_post_action_on_assoc(void *p_dm_void)
{
#if 0
	struct PHY_DM_STRUCT	*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_modify_RA_PCR_threshold(void *p_dm_void, u8 RA_offset_direction,
...
void phydm_rate_adaptive_mask_init(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_refresh_rate_adaptive_mask(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _rate_adaptive_table_ *p_ra_t = &p_dm->dm_ra_table;
...
void phydm_show_sta_info(void *p_dm_void, char input[][16], u32 *_used,
			 char *output, u32 *_out_len, u32 input_num)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "All STA: {1}\n"));
		PHYDM_SNPRINTF((output + used, out_len - used,
				"STA[macid]: {2} {macid}\n"));
		return;
	} else if (var1[0] == 1) {
	...
	for (i = macid_start; i < macid_end; i++) {
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				"ADDR:%x-%x-%x-%x-%x-%x\n", p_sta->mac_addr[5],
				p_sta->mac_addr[4], p_sta->mac_addr[3],
				p_sta->mac_addr[2], p_sta->mac_addr[1],
				p_sta->mac_addr[0]));
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				...
				p_sta->rssi_stat.rssi_ofdm,
				p_sta->rssi_stat.rssi_cck));

		/*[RA Info]*/
		PHYDM_SNPRINTF((output + used, out_len - used,
				"Rate_ID:%d, RSSI_LV:%d, ra_bw:%d, SGI_en:%d\n",
				p_ra->rate_id, p_ra->rssi_level,
				p_ra->ra_bw_mode, p_ra->is_support_sgi));

		PHYDM_SNPRINTF((output + used, out_len - used,
				"VHT_en:%d, Wireless_set=0x%x, sm_ps=%d\n",
				p_ra->is_vht_enable,
				p_sta->support_wireless_set, p_sta->sm_ps));

		PHYDM_SNPRINTF((output + used, out_len - used,
				"Dis{RA, PT}={%d, %d}, TxRx:%d, Noisy:%d\n",
				p_ra->disable_ra, p_ra->disable_pt,
				p_ra->txrx_state, p_ra->is_noisy));

		PHYDM_SNPRINTF((output + used, out_len - used,
				"TX{Rate, BW}={%d, %d}, RTY:%d\n",
				p_ra->curr_tx_rate, p_ra->curr_tx_bw,
				p_ra->curr_retry_ratio));
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				"TP{TX,RX}={%d, %d}\n",
				p_sta->tx_moving_average_tp,
				p_sta->rx_moving_average_tp));
	...
	if (tatal_sta_num == 0) {
		PHYDM_SNPRINTF(
			(output + used, out_len - used, "No Linked STA\n"));
	}
...
u8 phydm_get_tx_stream_num(void *p_dm_void, enum rf_type mimo_type
			   ...
			   )
{
	...
	if (mimo_type == RF_1T1R || mimo_type == RF_1T2R)
		tx_num = 1;
	else if (mimo_type == RF_2T2R || mimo_type == RF_2T3R ||
		 mimo_type == RF_2T4R)
		tx_num = 2;
...
u64 phydm_get_bb_mod_ra_mask(void *p_dm_void, u8 macid)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct cmn_sta_info *p_sta = p_dm->p_phydm_sta_info[macid];
	struct ra_sta_info *p_ra = NULL;
	...
	PHYDM_DBG(p_dm, DBG_RA,
		  ("macid=%d ori_RA_Mask= 0x%llx\n", macid, ra_mask_bitmap));
	PHYDM_DBG(
		p_dm, DBG_RA,
		("wireless_mode=0x%x, tx_stream_num=%d, BW=%d, MimoPs=%d, rssi_lv=%d\n",
	...
	if (wireless_mode == WIRELESS_CCK) /*B mode*/
		...
		ra_mask_bitmap &= 0x00000ff5;
		 ...
		 (WIRELESS_CCK | WIRELESS_OFDM | WIRELESS_HT)) {
		/*N_2G*/
		if (tx_stream_num == 1) {
		...
		if (tx_stream_num == 1) {
		   ...
		   (WIRELESS_CCK | WIRELESS_OFDM | WIRELESS_VHT)) {
		/*AC_2G*/
		if (tx_stream_num == 1)
		...
		if (bw ==
		    CHANNEL_WIDTH_20) { /* AC 20MHz doesn't support MCS9 */
...
u8 phydm_get_rate_id(void *p_dm_void, u8 macid)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct cmn_sta_info *p_sta = p_dm->p_phydm_sta_info[macid];
	struct ra_sta_info *p_ra = NULL;
	...
	PHYDM_DBG(p_dm, DBG_RA,
		  ("macid=%d, wireless_set=0x%x, tx_stream_num=%d, BW=0x%x\n",
	...
	if (wireless_mode == WIRELESS_CCK) /*B mode*/
		...
		if (tx_stream_num == 1)
		   ...
		   (WIRELESS_CCK | WIRELESS_OFDM | WIRELESS_HT)) { /*BGN mode*/
		...
		if (bw == CHANNEL_WIDTH_40) {
		   ...
		   (WIRELESS_OFDM | WIRELESS_VHT)) { /*AC mode*/
	...
	} else if (wireless_mode == (WIRELESS_CCK | WIRELESS_OFDM |
				     WIRELESS_VHT)) { /*AC 2.4G mode*/
...
void phydm_ra_h2c(void *p_dm_void, u8 macid, u8 dis_ra, u8 dis_pt,
		  u8 no_update_bw, u8 init_ra_lv, u64 ra_mask)
{
	...
	h2c_val[1] = (p_ra->rate_id & 0x1f) | ((init_ra_lv & 0x3) << 5) |
		     (p_ra->is_support_sgi << 7);
	h2c_val[2] =
		(u8)((p_ra->ra_bw_mode) | (((p_sta->ldpc_en) ? 1 : 0) << 2) |
...
void phydm_ra_registed(void *p_dm_void, u8 macid, u8 rssi_from_assoc)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _rate_adaptive_table_ *p_ra_t = &p_dm->dm_ra_table;
	struct cmn_sta_info *p_sta = p_dm->p_phydm_sta_info[macid];
	...
	{
		/*FW RA*/
		phydm_ra_h2c(p_dm, macid, p_ra->disable_ra, p_ra->disable_pt, 0,
			     init_ra_lv, ra_mask);
	}
}

void phydm_ra_offline(void *p_dm_void, u8 macid)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _rate_adaptive_table_ *p_ra_t = &p_dm->dm_ra_table;
	struct cmn_sta_info *p_sta = p_dm->p_phydm_sta_info[macid];
...
void phydm_ra_mask_watchdog(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _rate_adaptive_table_ *p_ra_t = &p_dm->dm_ra_table;
	struct cmn_sta_info *p_sta = NULL;
...
/*to be modified*/
		...
		rssi_lv_new = phydm_rssi_lv_dec(
			p_dm, (u32)p_sta->rssi_stat.rssi, p_ra->rssi_level);

		if ((p_ra->rssi_level != rssi_lv_new) ||
		    (p_ra_t->up_ramask_cnt >= FORCED_UPDATE_RAMASK_PERIOD)) {
			PHYDM_DBG(p_dm, DBG_RA_MASK,
				  ("RSSI LV:((%d))->((%d))\n", p_ra->rssi_level,
				   rssi_lv_new));
			...
			if (p_ra_t->record_ra_info)
				p_ra_t->record_ra_info(p_dm, macid, p_sta,
						       ra_mask);
			...
			{
				/*FW RA*/
				phydm_ra_h2c(p_dm, macid, p_ra->disable_ra,
					     p_ra->disable_pt, 1, 0, ra_mask);
			}
		}
	}
}
...
u8 phydm_vht_en_mapping(void *p_dm_void, u32 wireless_mode)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u8 vht_en_out = 0;
...
u8 phydm_rate_id_mapping(void *p_dm_void, u32 wireless_mode, u8 rf_type, u8 bw)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u8 rate_id_idx = 0;
...
void phydm_update_hal_ra_mask(void *p_dm_void, u32 wireless_mode, u8 rf_type,
			      ...
			      u8 tx_rate_level)
{
	...
	case PHYDM_WIRELESS_MODE_N_5G: {
		...
		} else if (rf_type == RF_2T2R || rf_type == RF_2T4R ||
			   rf_type == RF_2T3R) {
			if (bw == CHANNEL_WIDTH_40)
	...
	case PHYDM_WIRELESS_MODE_AC_24G: {
		if (rf_type == RF_1T1R)
			...
			ratr_bitmap_msb &= BBREG_0x3ff;
		}

		if (bw ==
		    CHANNEL_WIDTH_20) { /* AC 20MHz doesn't support MCS9 */
			ratr_bitmap &= 0x7fdfffff;
			ratr_bitmap_msb &= BBREG_0x1ff;
		}
	} break;
...
u8 phydm_rssi_lv_dec(void *p_dm_void, u32 rssi, u8 ratr_state)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u8 rssi_lv_table[RA_FLOOR_TABLE_SIZE] = {
		20, 34, 38, 42,
	...
	PHYDM_DBG(p_dm, DBG_RA_MASK,
		  ("curr RA level=(%d), Table_ori=[%d, %d, %d, %d, %d, %d]\n",
		   ratr_state, rssi_lv_table[0], rssi_lv_table[1],
		   rssi_lv_table[2], rssi_lv_table[3], rssi_lv_table[4],
		   rssi_lv_table[5]));
	...
	PHYDM_DBG(p_dm, DBG_RA_MASK,
		  ("RSSI=(%d), Table_mod=[%d, %d, %d, %d, %d, %d]\n", rssi,
		   rssi_lv_table[0], rssi_lv_table[1], rssi_lv_table[2],
		   rssi_lv_table[3], rssi_lv_table[4], rssi_lv_table[5]));
...
u8 phydm_rate_order_compute(void *p_dm_void, u8 rate_idx)
{
	u8 rate_order = 0;
...
u8 phydm_rate2ss(void *p_dm_void, u8 rate_idx)
{
	u8 ret = 0xff;
	u8 i, j;
	u8 search_idx;
...
u8 phydm_rate2plcp(void *p_dm_void, u8 rate_idx)
{
	...
	if (rate_idx < ODM_RATE6M) {
		...
		rate2ss = phydm_rate2ss(p_dm_void, rate_idx);
		if (rate2ss == 0xff)
			return 0xff;
		else
			ltftime = (rate2ss + 1) * 4;
...
u8 phydm_get_plcp(void *p_dm_void, u16 macid)
{
	u8 plcp_time = 0;
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct cmn_sta_info *p_sta = NULL;
	...
	p_sta = p_dm->p_phydm_sta_info[macid];
	p_ra = &p_sta->ra_info;
	plcp_time = phydm_rate2plcp(p_dm, p_ra->curr_tx_rate);
	return plcp_time;
}

void phydm_ra_common_info_update(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _rate_adaptive_table_ *p_ra_table = &p_dm->dm_ra_table;
	struct cmn_sta_info *p_sta = NULL;
	...
	for (macid = 0; macid < ODM_ASSOCIATE_ENTRY_NUM; macid++) {
		...
		if (is_sta_active(p_sta)) {
			...
			rate_order_tmp = phydm_rate_order_compute(
				p_dm, (p_sta->ra_info.curr_tx_rate & 0x7f));

			if (rate_order_tmp >=
			    (p_ra_table->highest_client_tx_order)) {
				p_ra_table->highest_client_tx_order =
					rate_order_tmp;
				p_ra_table->highest_client_tx_rate_order =
					macid;
			}
...
void phydm_ra_info_watchdog(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_ra_info_init(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _rate_adaptive_table_ *p_ra_table = &p_dm->dm_ra_table;
	...
	if (p_dm->support_ic_type == ODM_RTL8822B) {
		...
		ret_value = odm_get_bb_reg(p_dm, BBREG_0x4c8, MASKBYTE2);
		odm_set_bb_reg(p_dm, BBREG_0x4cc, MASKBYTE3, (ret_value - 1));
	}
#endif
...
u8 odm_find_rts_rate(void *p_dm_void, u8 tx_rate, boolean is_erp_protect)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	memcpy(&p_ra_table->per_rate_retrylimit_20M[0],
	       &per_rate_retrylimit_table_20M[0], ODM_NUM_RATE_IDX);
	memcpy(&p_ra_table->per_rate_retrylimit_40M[0],
	       &per_rate_retrylimit_table_40M[0], ODM_NUM_RATE_IDX);

	for (i = 0; i < ODM_NUM_RATE_IDX; i++) {
		phydm_retry_limit_table_bound(p_dm,
					      &p_ra_table->per_rate_retrylimit_20M[i],
					      0);
		phydm_retry_limit_table_bound(p_dm,
					      &p_ra_table->per_rate_retrylimit_40M[i],
					      0);
	}
}
...
)
{
	...
	if ((p_dm->rf_type == RF_2T2R) || (p_dm->rf_type == RF_2T3R) || (p_dm->rf_type == RF_2T4R)) {
		if ((p_dm->support_ic_type & (ODM_RTL8812 | ODM_RTL8192E)) &&
			...
			odm_set_mac_reg(p_dm, MACREG_0x4a4, MASKDWORD, 0xfc1ffff);	/*N-2SS, ARFR5, rate_id = 0xe*/
			odm_set_mac_reg(p_dm, MACREG_0x4a8, MASKDWORD, 0x0);		/*N-2SS, ARFR5, rate_id = 0xe*/
		} else if ((p_dm->support_ic_type & (ODM_RTL8812)) &&
			(wireless_mode & (ODM_WM_AC_5G | ODM_WM_AC_24G | ODM_WM_AC_ONLY))
			  ) {
			PHYDM_DBG(p_dm, DBG_RA, ("[ON ASSOC] set AC-2SS ARFR0 table\n"));
			odm_set_mac_reg(p_dm, MACREG_0x444, MASKDWORD, 0x0fff);	/*AC-2SS, ARFR0, rate_id = 0x9*/
			odm_set_mac_reg(p_dm, MACREG_0x448, MASKDWORD, 0xff01f000);		/*AC-2SS, ARFR0, rate_id = 0x9*/
		}
	}

...
void phydm_iq_gen_en(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		for (i = RF_PATH_A; i <= RF_PATH_B; i++) {
			odm_set_rf_reg(p_dm, (enum rf_path)i, RFREG_0xef,
				       BIT(19),
				       0x1); /*RF mode table write enable*/
			odm_set_rf_reg(p_dm, (enum rf_path)i, RFREG_0x33, 0xF,
				       3); /*Select RX mode*/
			odm_set_rf_reg(p_dm, (enum rf_path)i, RFREG_0x3e,
				       0xfffff, 0x00036); /*Set Table data*/
			odm_set_rf_reg(p_dm, (enum rf_path)i, RFREG_0x3f,
				       0xfffff, 0x5AFCE); /*Set Table data*/
			odm_set_rf_reg(p_dm, (enum rf_path)i, RFREG_0xef,
				       BIT(19),
				       0x0); /*RF mode table write disable*/
		}
	}
	#endif
#endif
...
void phydm_dis_cdd(void *p_dm_void)
{
#ifdef  PHYDM_COMPILE_IC_2SS
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	#if (ODM_IC_11AC_SERIES_SUPPORT)
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0x808, 0x3ffff00, 0);
		odm_set_bb_reg(p_dm, BBREG_0x9ac, 0x1fff, 0);
		odm_set_bb_reg(p_dm, BBREG_0x9ac, BIT(13), 1);
	}
	#endif
#endif
}

void phydm_pathb_q_matrix_rotate_en(void *p_dm_void)
{
#ifdef PHYDM_COMPILE_IC_2SS
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		if (phydm_api_trx_mode(p_dm, BB_PATH_AB, BB_PATH_AB, true) ==
		    false)
			return;
		#endif

		phydm_dis_cdd(p_dm);
		odm_set_bb_reg(p_dm, BBREG_0x195c, MASKDWORD,
			       0x40000); /*Set Q matrix r_v11 =1*/
		phydm_pathb_q_matrix_rotate(p_dm, 0);
		odm_set_bb_reg(p_dm, BBREG_0x191c, BIT(7),
			       1); /*Set Q matrix enable*/
	}
	#endif
#endif
}

void phydm_pathb_q_matrix_rotate(void *p_dm_void, u16 phase_idx)
{
#ifdef  PHYDM_COMPILE_IC_2SS
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	#if (ODM_IC_11AC_SERIES_SUPPORT)
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0x1954, 0xffffff,
			       phase_table_0[phase_idx]); /*Set Q matrix r_v21*/
		odm_set_bb_reg(p_dm, BBREG_0x1950, 0xffffff,
			       phase_table_1[phase_idx]);
	}
	#endif
#endif
}
...
void phydm_init_trx_antenna_setting(void *p_dm_void)
{
	...
	if (p_dm->support_ic_type & (ODM_RTL8814A)) {
		...
		rx_ant = (u8)odm_get_bb_reg(p_dm, ODM_REG(BB_RX_PATH, p_dm),
					    ODM_BIT(BB_RX_PATH, p_dm));
		tx_ant = (u8)odm_get_bb_reg(p_dm, ODM_REG(BB_TX_PATH, p_dm),
					    ODM_BIT(BB_TX_PATH, p_dm));
		p_dm->tx_ant_status = (tx_ant & 0xf);
		p_dm->rx_ant_status = (rx_ant & 0xf);
	} else if (p_dm->support_ic_type &
		   (ODM_RTL8723D | ODM_RTL8821C |
...
void phydm_config_ofdm_tx_path(void *p_dm_void, u32 path)
{
	...
	if (p_dm->support_ic_type & (ODM_RTL8192E)) {
		if (path == BB_PATH_A) {
			odm_set_bb_reg(p_dm, BBREG_0x90c, MASKDWORD, 0x81121111);
			/**/
		} else if (path == BB_PATH_B) {
			odm_set_bb_reg(p_dm, BBREG_0x90c, MASKDWORD, 0x82221222);
			/**/
		} else  if (path == BB_PATH_AB) {
			odm_set_bb_reg(p_dm, BBREG_0x90c, MASKDWORD, 0x83321333);
			/**/
		}
	...
	if (p_dm->support_ic_type & (ODM_RTL8812)) {
		...
		odm_set_bb_reg(p_dm, BBREG_0x80c, 0xff00, ofdm_tx_path);
	}
#endif
}
...
void phydm_config_ofdm_rx_path(void *p_dm_void, u32 path)
{
	...
	if (p_dm->support_ic_type & (ODM_RTL8192E)) {
		...
		odm_set_bb_reg(p_dm, BBREG_0xc04, 0xff, (((ofdm_rx_path) << 4) | ofdm_rx_path));
		odm_set_bb_reg(p_dm, BBREG_0xd04, 0xf, ofdm_rx_path);
#endif
	}
...
void phydm_config_cck_rx_antenna_init(void *p_dm_void)
{
	...
	odm_set_bb_reg(p_dm, BBREG_0xa00, BIT(15),
		       0x0); /*Disable antenna diversity*/
	odm_set_bb_reg(p_dm, BBREG_0xa70, BIT(7),
		       0); /*Concurrent CCA at LSB & USB*/
	odm_set_bb_reg(p_dm, BBREG_0xa74, BIT(8),
		       0); /*RX path diversity enable*/
	odm_set_bb_reg(p_dm, BBREG_0xa14, BIT(7), 0); /*r_en_mrc_antsel*/
	odm_set_bb_reg(p_dm, BBREG_0xa20, (BIT(5) | BIT(4)),
		       1); /*MBC weighting*/

	if (p_dm->support_ic_type & (ODM_RTL8192E | ODM_RTL8197F)) {
		odm_set_bb_reg(p_dm, BBREG_0xa08, BIT(28),
			       1); /*r_cck_2nd_sel_eco*/
		/**/
	} else if (p_dm->support_ic_type & ODM_RTL8814A) {
		odm_set_bb_reg(p_dm, BBREG_0xa84, BIT(28), 1); /*2R CCA only*/
		/**/
	}
#endif
}
...
void phydm_config_cck_rx_path(void *p_dm_void, enum bb_path path)
{
	...
	odm_set_bb_reg(p_dm, BBREG_0xa04, (BIT(27) | BIT(26)),
		       cck_path[0]); /*CCK_1 input signal path*/
	odm_set_bb_reg(p_dm, BBREG_0xa04, (BIT(25) | BIT(24)),
		       cck_path[1]); /*CCK_2 input signal path*/
	odm_set_bb_reg(p_dm, BBREG_0xa74, BIT(8),
		       path_div_select); /*enable Rx path diversity*/
	odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(18),
		       en_2R_path); /*enable 2R Rx path*/
	odm_set_bb_reg(p_dm, BBREG_0xa2c, BIT(22), en_2R_mrc); /*enable 2R MRC*/

#endif
}

void phydm_config_trx_path(void *p_dm_void, u32 *const dm_value, u32 *_used,
			   char *output, u32 *_out_len)
{
	...
	if (dm_value[0] == 0) {
		if (dm_value[1] == 1) { /*TX*/
			if (dm_value[2] == 1)
				odm_set_bb_reg(p_dm, BBREG_0xa04, 0xf0000000,
					       0x8);
...
void phydm_stop_3_wire(void *p_dm_void, u8 set_type)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (set_type == PHYDM_SET) {
		/*[Stop 3-wires]*/
		if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
			odm_set_bb_reg(p_dm, BBREG_0xc00, 0xf,
				       0x4); /*	hardware 3-wire off */
			odm_set_bb_reg(p_dm, BBREG_0xe00, 0xf,
				       0x4); /*	hardware 3-wire off */
		} else {
			odm_set_bb_reg(
				p_dm, BBREG_0x88c, 0xf00000,
		...
		if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
			odm_set_bb_reg(p_dm, BBREG_0xc00, 0xf,
				       0x7); /*	hardware 3-wire on */
			odm_set_bb_reg(p_dm, BBREG_0xe00, 0xf,
				       0x7); /*	hardware 3-wire on */
		} else {
			odm_set_bb_reg(p_dm, BBREG_0x88c, 0xf00000,
				       0x0); /* 3 wire enable 88c[23:20]=0x0 */
		}
	}
}

u8 phydm_stop_ic_trx(void *p_dm_void, u8 set_type)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (set_type == PHYDM_SET) {
		/*[Stop TRX]---------------------------------------------------------------------*/
		if (phydm_set_bb_dbg_port(p_dm, BB_DBGPORT_PRIORITY_3, 0x0) ==
		    false) /*set debug port to 0x0*/
		...
		if (trx_idle_success) {
			p_api->tx_queue_bitmap =
				(u8)odm_get_bb_reg(p_dm, BBREG_0x520, 0xff0000);

			odm_set_bb_reg(p_dm, BBREG_0x520, 0xff0000,
				       0xff); /*pause all TX queue*/

			if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
				odm_set_bb_reg(p_dm, BBREG_0x808, BIT(28),
					       0); /*disable CCK block*/
				odm_set_bb_reg(p_dm, BBREG_0x838, BIT(1),
					       1); /*disable OFDM RX CCA*/
			} else {
				/*TBD*/
				odm_set_bb_reg(p_dm, BBREG_0x800, BIT(24),
					       0); /* disable whole CCK block */

				p_api->rx_iqc_reg_1 = odm_get_bb_reg(
					p_dm, BBREG_0xc14, MASKDWORD);
				p_api->rx_iqc_reg_2 = odm_get_bb_reg(
					p_dm, BBREG_0xc1c, MASKDWORD);

				odm_set_bb_reg(
					p_dm, BBREG_0xc14, MASKDWORD,
					0x0); /* [ Set IQK Matrix = 0 ] equivalent to [ Turn off CCA] */
				odm_set_bb_reg(p_dm, BBREG_0xc1c, MASKDWORD,
					       0x0);
			}
		...
		odm_set_bb_reg(
			p_dm, BBREG_0x520, 0xff0000,
			(u32)(p_api->tx_queue_bitmap)); /*Release all TX queue*/

		if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
			odm_set_bb_reg(p_dm, BBREG_0x808, BIT(28),
				       1); /*enable CCK block*/
			odm_set_bb_reg(p_dm, BBREG_0x838, BIT(1),
				       0); /*enable OFDM RX CCA*/
		} else {
			/*TBD*/
			odm_set_bb_reg(p_dm, BBREG_0x800, BIT(24),
				       1); /* enable whole CCK block */

			odm_set_bb_reg(
				p_dm, BBREG_0xc14, MASKDWORD,
				p_api->rx_iqc_reg_1); /* [ Set IQK Matrix = 0 ] equivalent to [ Turn off CCA] */
			odm_set_bb_reg(p_dm, BBREG_0xc1c, MASKDWORD,
				       p_api->rx_iqc_reg_2);
		}
			  ...
			  char *output, u32 *_out_len)
{
	...
	if (p_dm->support_ic_type & (ODM_RTL8821 | ODM_RTL8881A)) {
		...
		odm_set_bb_reg(p_dm, BBREG_0xcb4, 0xFF, 77); /*DPDT_N = 1b'0*/  /*DPDT_P = 1b'0*/

		if (ext_ant_switch == MAIN_ANT) {
			odm_set_bb_reg(p_dm, BBREG_0xcb4, (BIT(29) | BIT(28)), 1);
			PHYDM_DBG(p_dm, ODM_COMP_API, ("***8821A set ant switch = 2b'01 (Main)\n"));
		} else if (ext_ant_switch == AUX_ANT) {
			odm_set_bb_reg(p_dm, BBREG_0xcb4, BIT(29) | BIT(28), 2);
...
void phydm_csi_mask_enable(void *p_dm_void, u32 enable)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u32 reg_value = 0;
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0xd2c, BIT(28), reg_value);
		PHYDM_DBG(p_dm, ODM_COMP_API,
			  ("Enable CSI Mask:  Reg BBREG_0xd2c[28] = ((0x%x))\n",
			   reg_value));

	} else if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0x874, BIT(0), reg_value);
		PHYDM_DBG(p_dm, ODM_COMP_API,
			  ("Enable CSI Mask:  Reg BBREG_0x874[0] = ((0x%x))\n",
			   reg_value));
	}
}

void phydm_clean_all_csi_mask(void *p_dm_void)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;

	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0xd40, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0xd44, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0xd48, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0xd4c, MASKDWORD, 0);

	} else if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0x880, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0x884, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0x888, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0x88c, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0x890, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0x894, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0x898, MASKDWORD, 0);
		odm_set_bb_reg(p_dm, BBREG_0x89c, MASKDWORD, 0);
	}
}

void phydm_set_csi_mask_reg(void *p_dm_void, u32 tone_idx_tmp,
			    u8 tone_direction)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		tone_num = 64;
		csi_mask_reg_p = BBREG_0xd40;
		csi_mask_reg_n = BBREG_0xd48;

	} else if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		tone_num = 128;
		csi_mask_reg_p = BBREG_0x880;
		csi_mask_reg_n = BBREG_0x890;
	}
...
void phydm_set_nbi_reg(void *p_dm_void, u32 tone_idx_tmp, u32 bw)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0xc40, 0x1f000000, reg_idx);
		PHYDM_DBG(p_dm, ODM_COMP_API,
			  ("Set tone idx:  Reg0xC40[28:24] = ((0x%x))\n",
			   reg_idx));
		/**/
	} else {
		odm_set_bb_reg(p_dm, BBREG_0x87c, 0xfc000, reg_idx);
		PHYDM_DBG(p_dm, ODM_COMP_API,
...
void phydm_nbi_enable(void *p_dm_void, u32 enable)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u32 reg_value = 0;
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0xc40, BIT(9), reg_value);
		PHYDM_DBG(p_dm, ODM_COMP_API,
			  ("Enable NBI Reg0xC40[9] = ((0x%x))\n", reg_value));

	} else if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		if (p_dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C)) {
			odm_set_bb_reg(p_dm, BBREG_0x87c, BIT(13), reg_value);
			odm_set_bb_reg(p_dm, BBREG_0xc20, BIT(28), reg_value);
			if (p_dm->rf_type > RF_1T1R)
				odm_set_bb_reg(p_dm, BBREG_0xe20, BIT(28),
					       reg_value);
		} else
			odm_set_bb_reg(p_dm, BBREG_0x87c, BIT(13), reg_value);
		PHYDM_DBG(p_dm, ODM_COMP_API,
...
u8 phydm_calculate_fc(void *p_dm_void, u32 channel, u32 bw, u32 second_ch,
		      u32 *fc_in)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u32 fc = *fc_in;
...
u8 phydm_calculate_intf_distance(void *p_dm_void, u32 bw, u32 fc,
...
u8 phydm_csi_mask_setting(void *p_dm_void, u32 enable, u32 channel, u32 bw,
			  u32 f_interference, u32 second_ch)
{
	...
	if (enable == FUNC_DISABLE) {
		...
		/*calculate fc*/
		if (phydm_calculate_fc(p_dm, channel, bw, second_ch, &fc) ==
		    PHYDM_SET_FAIL)
			...
			if (phydm_calculate_intf_distance(
				    p_dm, bw, fc, f_interference,
				    &tone_idx_tmp) == PHYDM_SET_SUCCESS) {
				tone_direction = (f_interference >= fc) ?
							 FREQ_POSITIVE :
							 FREQ_NEGATIVE;
				phydm_set_csi_mask_reg(p_dm, tone_idx_tmp,
						       tone_direction);
				set_result = PHYDM_SET_SUCCESS;
			} else
...
u8 phydm_nbi_setting(void *p_dm_void, u32 enable, u32 channel, u32 bw,
		     u32 f_interference, u32 second_ch)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u32 fc = 2412;
...
void phydm_api_debug(void *p_dm_void, u32 function_map, u32 *const dm_value,
	...
	if (function_map == PHYDM_API_NBI) {
		if (dm_value[0] == 100) {
				...
				((second_ch == PHYDM_DONT_CARE) || (bw == 20) ||
				 (channel > 14)) ?
					...
					((second_ch == PHYDM_ABOVE) ? "H" :
								      "L")));
			set_result = phydm_nbi_setting(
				p_dm, FUNC_ENABLE, channel, bw, f_interference,
				second_ch);
			...
			PHYDM_SNPRINTF((output + used, out_len - used,
					"[Disable NBI]\n"));
			set_result = phydm_nbi_setting(
				p_dm, FUNC_DISABLE, channel, bw, f_interference,
				second_ch);
...
void phydm_stop_ck320(void *p_dm_void, u8 enable)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u32 reg_value = enable ? 1 : 0;

	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_reg(p_dm, BBREG_0x8b4, BIT(6), reg_value);
		/**/
	} else {

		if (p_dm->support_ic_type & ODM_IC_N_2SS) { /*N-2SS*/
			odm_set_bb_reg(p_dm, BBREG_0x87c, BIT(29), reg_value);
			/**/
		} else { /*N-1SS*/
			odm_set_bb_reg(p_dm, BBREG_0x87c, BIT(31), reg_value);
			/**/
		}
	}
}

boolean phydm_set_bb_txagc_offset(void *p_dm_void,
				  ...
				  )
{
	...
	PHYDM_DBG(p_dm, ODM_COMP_API, ("power_offset=%d, add_half_db =%d\n",
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		if (power_offset > -16 || power_offset < 15) {
			...
			PHYDM_DBG(p_dm, ODM_COMP_API,
				  ("Reg_idx =0x%x\n", power_idx));
			odm_set_bb_reg(p_dm, BBREG_0x8b4, 0x3f, power_idx);
			set_success = true;
		} else {
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		if (power_offset > -8 || power_offset < 7) {
			...
			PHYDM_DBG(p_dm, ODM_COMP_API,
				  ("Reg_idx =0x%x\n", power_idx));
			odm_set_bb_reg(p_dm, BBREG_0x80c, 0x1f00, power_idx);
			odm_set_bb_reg(p_dm, BBREG_0x80c, 0x3e000, power_idx);
			set_success = true;
		} else {
			dbg_print("[Warning] TX AGC Offset Setting error!");
		}
...
#ifdef PHYDM_COMMON_API_SUPPORT
boolean phydm_api_set_txagc(void *p_dm_void, u32 power_index, enum rf_path path,
...
u8 phydm_api_get_txagc(void *p_dm_void, enum rf_path path, u8 hw_rate)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	u8 ret = 0;
...
boolean phydm_api_switch_bw_channel(void *p_dm_void, u8 central_ch,
				    u8 primary_ch_idx,
				    enum channel_width bandwidth)
{
	...
	if (p_dm->support_ic_type & ODM_RTL8822B)
		ret = config_phydm_switch_channel_bw_8822b(
			p_dm, central_ch, primary_ch_idx, bandwidth);
#endif
...
boolean phydm_api_trx_mode(void *p_dm_void, enum bb_path tx_path,
...
#if (RTL8822B_SUPPORT == 1)
	if (p_dm->support_ic_type & ODM_RTL8822B)
		ret = config_phydm_trx_mode_8822b(p_dm, tx_path, rx_path,
						  is_tx2_path);
#endif
